...
#[test]
fn read_capability_accepts_a_trimmed_graphic_token() {
    let (read_end, write_end) = pipe_pair();
    {
        use std::io::Write;
        let mut writer = std::fs::File::from(write_end);
        writer.write_all(b"capability-token-1\n").expect("write");
    }
    let value = read_capability(read_end).expect("valid capability");
    assert_eq!(value, "capability-token-1");
}

#[test]
fn read_capability_rejects_empty_and_oversized_content() {
    let (empty_read, empty_write) = pipe_pair();
    drop(empty_write);
    assert!(read_capability(empty_read).is_err());

    let (big_read, big_write) = pipe_pair();
    {
        use std::io::Write;
        let mut writer = std::fs::File::from(big_write);
        writer.write_all(&vec![b'a'; 300]).expect("write");
    }
    // `take(257)` caps the read itself, so this proves the length rejection
    // rather than an unbounded read.
    assert!(read_capability(big_read).is_err());
}

#[test]
fn read_capability_rejects_non_graphic_bytes() {
    let (read_end, write_end) = pipe_pair();
    {
        use std::io::Write;
        let mut writer = std::fs::File::from(write_end);
    ...
    assert!(read_capability(read_end).is_err());
}
