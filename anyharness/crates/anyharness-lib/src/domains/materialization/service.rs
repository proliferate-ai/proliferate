...
impl MaterializationService {
    ...
    pub async fn acquire_repo_root(
        ...
        let (_guard, recovered_intended_kind) = match self.begin_operation(
            &operation_id,
    ...
    /// needed — replay is a pure read). Otherwise takes the in-process guard
    /// FIRST, then reconciles the ledger row while holding it:
    ///   - a `running` row with the lock free is a *crashed* op we adopt on
    ///     retry (its `intended_kind`, if any, is recovered);
    ///   - a live concurrent same-id caller cannot take the guard and is
    ///     rejected with a retryable `OperationConflict`.
    fn begin_operation(
        &self,
        operation_id: &str,
...
/// Acquire the in-process lock and reconcile the ledger for an operation.
///
/// A completed op replays without contending for the lock (a pure read).
/// Otherwise the in-process guard is taken FIRST — a live concurrent same-id
/// caller cannot take it and is rejected with a retryable `OperationConflict`,
/// while a crashed op (running row, no in-process holder) is adopted on retry
/// with its recorded `intended_kind` recovered. Shared by the service methods
/// and the concurrency tests so both drive one implementation.
pub(crate) fn begin_operation(
    ...
    // A completed op replays without contending for the lock.
    ...
    // Claim the id in-process. A live concurrent runner already holds it.
    let guard = operation_locks.try_acquire(operation_id).ok_or_else(|| {
        MaterializationError::OperationConflict(
            "an identical materialization operation is already in progress".into(),
        )
    })?;

    // Re-read under the guard: reconcile the durable row now that we are the
    // sole live in-process runner for this id.
    ...
    if let Some(existing) = store.find(operation_id).map_err(internal)? {
        match admit_existing(&existing, kind, request_hash)? {
            ...
            Admission::Retry => {
                // Crashed row (no in-process holder): recover its recorded clone
