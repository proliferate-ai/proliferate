...
fn preexisting_empty_destination_survives_identity_mismatch() {
    ...
    let dest_target = dest.path().join("widget");
    // The user pre-creates the (empty) destination directory themselves.
    std::fs::create_dir_all(&dest_target).expect("pre-create empty target");
    let home = Guard::new("preexisting-empty-mismatch-home");
    let config_home = Guard::new("preexisting-empty-mismatch-gitconfig");
    ...
    let repo_root_service = RepoRootService::new(RepoRootStore::new(db.clone()));
    let runtime = make_runtime(&db, home.path());
    // A wrong expected identity forces post-clone verification to fail.
    let wrong = RemoteIdentity::new("github", "someone-else", "different");
    ...
    let error = error.expect_err("identity mismatch must fail");
    assert_eq!(error.code(), "REPOSITORY_REMOTE_MISMATCH");
    // The user's pre-existing directory must still exist (we did not create it).
    assert!(
        dest_target.exists(),
