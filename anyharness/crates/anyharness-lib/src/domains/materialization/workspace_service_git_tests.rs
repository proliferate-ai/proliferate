...
async fn make_app_state(prefix: &str) -> (tokio::sync::MutexGuard<'static, ()>, Guard, AppState) {
    let env = crate::app::test_support::lock_env().await;
...
async fn omitted_destination_crash_retry_adopts_the_recorded_checkout() {
    let (_env, _home, state) = make_app_state("crash-home").await;
...
async fn explicit_busy_destination_returns_workspace_busy() {
    let (_env, _home, state) = make_app_state("busy-home").await;
...
async fn explicit_busy_destination_with_active_terminal_returns_workspace_busy() {
    let (_env, _home, state) = make_app_state("busy-terminal-home").await;
...
async fn completed_replay_rejects_malformed_observed_head_sha() {
    let (_env, _home, state) = make_app_state("replay-home").await;
