...
async fn every_workspace_tool_accepts_one_schema_valid_call_through_the_dispatcher() {
    ...
    for (id, case) in cases.into_iter().enumerate() {
        ...
        match case.outcome {
            ValidCallOutcome::Success => {
                assert_eq!(
                    result["isError"], false,
                    "{} rejected schema-valid arguments: {response}",
                    case.name
                );
                // MCP requires an object here; a bare array or scalar is not a
                // valid structured result for any tool.
                assert!(
                    result["structuredContent"].is_object(),
                    "{} returned non-object structuredContent: {response}",
                    case.name
                );
            }
...
async fn list_subagents_returns_the_agent_page_object() {
    ...
    let response = dispatch_tool_call(&server, &token, 1, "list_subagents", json!({})).await;
    let structured = &response["result"]["structuredContent"];
    assert!(structured.is_object(), "{response}");
    assert!(structured["agents"].is_array(), "{response}");
}
