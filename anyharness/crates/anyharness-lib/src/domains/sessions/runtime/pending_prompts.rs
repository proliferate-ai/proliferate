...
impl SessionRuntime {
    pub async fn edit_pending_prompt(
        ...
        handle
            ...
            .map_err(|error| match error {
                ...
                LiveSessionCommandError::Rejected(QueueMutationError::SteerInterruptFailed) => {
                    PendingPromptMutationError::Internal(anyhow::anyhow!(
                        "unexpected steer interruption failure while editing a pending prompt"
                    ))
                }
    ...
    pub async fn delete_pending_prompt(
        ...
        handle
            ...
            .map_err(|error| match error {
                ...
                LiveSessionCommandError::Rejected(QueueMutationError::SteerInterruptFailed) => {
                    PendingPromptMutationError::Internal(anyhow::anyhow!(
                        "unexpected steer interruption failure while deleting a pending prompt"
                    ))
                }
                LiveSessionCommandError::Rejected(QueueMutationError::Internal(error)) => {
                    PendingPromptMutationError::Internal(anyhow::anyhow!(error))
    ...
    pub async fn reorder_pending_prompts(
        ...
        if let Some(run_id) = self.workflow_held_run(session_id) {
            return Err(PendingPromptQueueError::WorkflowHeld { run_id });
        }
        self.access_gate
            .assert_can_mutate_for_session(session_id)
            .map_err(PendingPromptQueueError::Access)?;
        let record = self
            .get_session_or_not_found(session_id)
...
fn map_queue_lifecycle_error(error: SessionLifecycleError) -> PendingPromptQueueError {
    match error {
        ...
        SessionLifecycleError::Access(error) => PendingPromptQueueError::Access(error),
        SessionLifecycleError::WorkflowHeld { run_id } => {
            PendingPromptQueueError::WorkflowHeld { run_id }
        }
...
fn map_queue_command_error(
    ...
    match error {
        ...
        LiveSessionCommandError::Rejected(QueueMutationError::SteerInterruptFailed) => {
            PendingPromptQueueError::SteerInterruptFailed
        }
        LiveSessionCommandError::Rejected(QueueMutationError::Internal(error)) => {
