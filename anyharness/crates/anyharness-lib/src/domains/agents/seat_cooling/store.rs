...
impl SeatCoolingStore {
    ...
    /// Record (or extend) a seat's cooling deadline. Upsert: a later
    /// observation replaces the row — the newest limit error is authoritative.
    pub fn mark_cooling(
        &self,
        seat_id: &str,
        harness_kind: &str,
            ...
            conn.execute(
                "INSERT INTO seat_cooling (
                 ...
                 ON CONFLICT(seat_id) DO UPDATE SET
                    ...
                    cooling_until_epoch_s = excluded.cooling_until_epoch_s,
