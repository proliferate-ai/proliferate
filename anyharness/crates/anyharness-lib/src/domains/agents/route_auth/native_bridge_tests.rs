...
use super::super::{
    render_profile, resolve_profile_bridged, AbsentHarnessPolicy, GatewayModelPlan, RouteAuthError,
};
use super::*;
...
fn the_migration_grants_only_absent_natively_authenticated_harnesses() {
    ...
    assert_eq!(
        outcome,
        NativeBridgeSeedOutcome::Seeded {
            harnesses: expected.clone()
        }
    );
    assert_eq!(granted(&home), expected);
    assert!(
        !launch_native_grant(&home, "claude"),
        "configured harness is never bridged"
    );
    assert!(
        !launch_native_grant(&home, "grok"),
        "present-but-empty is a selection, not bridged"
    );
    assert!(
        !launch_native_grant(&home, "cursor"),
        "absent but not logged in: nothing to keep"
    );
...
fn no_state_file_means_every_native_login_is_bridged() {
    ...
    let expected: BTreeSet<String> = ["claude", "opencode"]
        .iter()
        .map(|k| k.to_string())
        .collect();
    assert_eq!(
...
fn pre_cutover_native_user_first_launch_prompts_not_refuses() {
    ...
    assert!(
        matches!(refused, RouteAuthError::NoConfiguredSource { ref harness_kind } if harness_kind == "codex")
    );
    assert!(
        refused.to_string().contains("isn't set up"),
        "refusal speaks plain words: {refused}"
    );
...
fn the_bridge_never_overrides_an_explicit_selection() {
    ...
    let error =
        resolve_profile_bridged(Some(&applied), "claude", AbsentHarnessPolicy::Refuse, true)
...
fn legacy_flag_launch_keeps_native_behavior() {
    ...
    let profile = resolve_profile_bridged(None, "claude", AbsentHarnessPolicy::Refuse, true)
        .expect("bridged");
    let rendered =
        render_profile(&profile, "claude", &GatewayModelPlan::default(), &home).expect("render");
...
fn acting_on_the_prompt_clears_the_flag() {
    ...
    assert!(
        !clear_native_bridge_flag(&home, "grok").expect("clear again"),
        "second clear is inert"
    );
    ...
    assert_eq!(
        seed(&home, None, &["claude"]),
        NativeBridgeSeedOutcome::AlreadySeeded
    );
    let _ = fs::remove_dir_all(home);
}
...
fn clearing_on_an_unseeded_home_is_inert() {
    ...
    clear_native_bridge_flags_for_document(&home, &document(&[("claude", true)]))
        .expect("clear doc");
    assert!(
        !native_bridge_path(&home).exists(),
        "clearing never creates the marker"
    );
...
fn an_unknown_version_bridge_is_never_overwritten_and_fails_open() {
    ...
    assert_eq!(
        seed(&home, None, &["codex"]),
        NativeBridgeSeedOutcome::AlreadySeeded
    );
    assert!(launch_native_grant(&home, "claude"));
    assert!(
        launch_native_grant(&home, "codex"),
        "unknown contents fail open for every kind"
    );
    assert!(granted(&home).is_empty());
    assert!(
        !clear_native_bridge_flag(&home, "claude").expect("clear"),
        "dismiss is inert"
    );
    assert_eq!(
        fs::read(&path).unwrap(),
        newer.to_vec(),
        "the newer file is byte-identical"
    );
    let _ = fs::remove_dir_all(home);
}
