...
// --- server-switch origin guard --------------------------------------------
// (self-hosting-v1 §3.5, D1): a state file stamped for a different server
...
// (not the env-var-reading public wrapper) so these run safely alongside
// every other test in this crate without mutating process-global env state.

fn stamped_gateway_state(kind: &str, issuing_server_origin: Option<&str>) -> Value {
    let mut state = gateway_state(kind);
