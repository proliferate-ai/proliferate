...
#[test]
fn failed_interrupt_retry_skips_a_second_promotion() {
    let first_order = plan_steer_order(&[1, 2], 2)
        .expect("target exists")
        .expect("first steer must promote");
    assert_eq!(first_order, vec![2, 1]);
    assert!(matches!(
        map_steer_interrupt_result::<(), _>(Err("connection closed")),
        Err(QueueMutationError::SteerInterruptFailed)
    ));

    // Durable state already reflects the first promotion. Retrying the same
    // stable queue id only retries cancellation; it cannot reorder or emit a
    // second promotion event.
    assert!(plan_steer_order(&first_order, 2)
        .expect("target still exists")
        .is_none());
    assert!(map_steer_interrupt_result::<(), &str>(Ok(())).is_ok());
}
