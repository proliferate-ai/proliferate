...
impl SessionActor {
    ...
    pub(in crate::live::sessions::actor) async fn handle_steer_pending_prompt(
        ...
        if let Some(desired_seqs) = plan_steer_order(&expected_seqs, seq)? {
            self.persist_pending_prompt_order(&expected_seqs, &desired_seqs)
                .await?;
        }
        ...
        if is_busy {
            ...
            map_steer_interrupt_result(cancel_result)?;
        }
        Ok(())
    }
...
pub(in crate::live::sessions::actor) fn plan_steer_order(
    current_seqs: &[i64],
    seq: i64,
) -> Result<Option<Vec<i64>>, QueueMutationError> {
    if !current_seqs.contains(&seq) {
        return Err(QueueMutationError::NotFound);
    }
    if current_seqs.first().copied() == Some(seq) {
        // A retry after a degraded steer must not persist or emit a second
        // reorder. It proceeds directly to the cancellation notification.
        return Ok(None);
    }
    let mut desired_seqs = Vec::with_capacity(current_seqs.len());
    ...
    Ok(Some(desired_seqs))
}

pub(in crate::live::sessions::actor) fn map_steer_interrupt_result<T, E: std::fmt::Display>(
    result: Result<T, E>,
) -> Result<(), QueueMutationError> {
    result.map(|_| ()).map_err(|error| {
        tracing::warn!(error = %error, "pending prompt was promoted but active turn interruption failed");
        QueueMutationError::SteerInterruptFailed
    })
}
