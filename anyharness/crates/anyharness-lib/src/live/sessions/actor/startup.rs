...
pub(in crate::live::sessions::actor) fn action_capabilities_from_acp(
    ...
    SessionActionCapabilities {
        fork,
        targeted_fork: false,
        supports_goals: anyharness_ext_capability_supported(init_response.meta.as_ref(), "goals"),
        supports_loops: anyharness_ext_capability_supported(init_response.meta.as_ref(), "loops"),
    }
}

/// Wire-contract capability advertisement (goals/loops):
/// `InitializeResponse._meta.anyharness.{goals,loops}.supported == true`,
/// gated on `schemaVersion == 1`.
fn anyharness_ext_capability_supported(
