...
impl IdleReapPolicy {
    ...
    pub fn from_env() -> Self {
        let Some(raw) = std::env::var_os(IDLE_REAP_SECONDS_ENV) else {
...
pub(crate) struct IdleSweepOutcome {
    ...
    /// Sessions held back only because a human has not answered a permission
    /// or input request. Counted so the abandoned-prompt leak is measurable
...
impl IdleSessionReaper {
    ...
    pub(crate) async fn sweep(&mut self, now: Instant) -> IdleSweepOutcome {
        ...
        for (session_id, handle) in live {
            ...
            if verdict == IdleReapVerdict::AwaitingInteraction {
                outcome.awaiting_interaction_held += 1;
            }
            ...
            match self.manager.unload_session_nonterminal(&session_id).await {
                Ok(()) => {
