...
/// Registration order is the observer dispatch order: plans run first. Goals
/// consume and feed nothing in-pass, so they run after plans; loops and
/// activity are registered after goals per the session-activity-architecture
/// build order (both also consume and feed nothing in-pass, so their relative
/// order is unconstrained beyond "after goals").
