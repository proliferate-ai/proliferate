...
use super::command::{InteractionResolution, ResolveInteractionCommandError, SessionCommand};
...
pub struct LiveSessionHandle {
    ...
    pub(super) execution: Arc<RwLock<LiveSessionExecutionSnapshot>>,
    pub(super) native_session_id: Arc<std::sync::RwLock<Option<String>>>,
}
...
impl LiveSessionHandle {
    ...
    pub(crate) fn set_native_session_id(&self, native_session_id: String) {
        self.native_session_id
            .write()
            .expect("native session id lock poisoned")
            .replace(native_session_id);
    }
