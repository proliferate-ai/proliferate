use anyharness_contract::v1::{
    ...
    AgentLaunchOptionsResponse, AgentModelRegistryModel, AgentModelRegistrySnapshotResponse,
    AgentReadinessState, AgentSummary, ArtifactStatus, InstallAgentRequest, InstallAgentResponse,
    LoginCommand, ModelCatalogStatus as ContractModelCatalogStatus, ModelRegistrySource,
    ModelRegistryStatus, ProblemDetails, ReconcileAgentResult, ReconcileAgentsRequest,
    ReconcileAgentsResponse, ReconcileJobStatus, ReconcileOutcome,
    RefreshAgentModelRegistryRequest, RefreshAgentModelRegistryResponse, StartAgentLoginRequest,
...
use crate::domains::agents::model_registry::projection::bundled_registry_for_kind;
...
pub async fn get_agent_model_registry(
    ...
    Ok(Json(to_model_registry_snapshot_response(
        snapshot.unwrap_or_else(|| bundled_snapshot_for_kind(&kind, query.workspace_id)),
    )))
}
...
fn bundled_snapshot_for_kind(
    kind: &str,
    workspace_id: Option<String>,
) -> DynamicModelRegistrySnapshot {
    let registry = bundled_registry_for_kind(kind).unwrap_or_else(|| {
        crate::domains::agents::model::ModelRegistryMetadata {
            kind: kind.to_string(),
    ...
    DynamicModelRegistrySnapshot {
        kind: registry.kind,
        workspace_id,
        source: DynamicModelRegistrySource::BundledCatalog,
        ...
        models: registry
            ...
            .into_iter()
            .map(
                |model| crate::domains::agents::model_registry::model::DynamicModelRegistryModel {
                    id: model.id,
                    display_name: model.display_name,
