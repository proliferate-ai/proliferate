...
mod tests {
    ...
    fn temp_path(file_name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir()
            .join(format!("anyharness-file-logging-{unique}"))
            .join(file_name)
    }

    #[test]
    fn rotating_writer_creates_file_on_first_write() {
        let path = temp_path("anyharness.log");
        let mut writer = build_rotating_writer(&path).expect("writer should be created");
        writeln!(writer, "hello world").expect("write should succeed");
    ...
    fn rotating_writer_does_not_rotate_below_threshold() {
        ...
        let mut writer = build_rotating_writer(&path).expect("writer should be created");
        for _ in 0..100 {
            writeln!(writer, "small log line").expect("write should succeed");
        }
        writer.flush().expect("flush should succeed");
    ...
    fn create_file_log_sink_errors_when_parent_is_not_directory() {
        ...
        let log_path = parent.join("anyharness.log");
