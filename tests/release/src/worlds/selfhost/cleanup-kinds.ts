...
export type SelfHostCleanupResourceKind = Extract<
  ...
  | "run_directory"
  | "port_registration"
  // Formerly SH-CLOUD-ADDON only (SELFHOST-QUAL-1). That cell was deleted with
  // the cloud sandbox stack (cull part 2) — no self-host cell registers these
  // kinds anymore; they are retained so replay of an old persisted ledger
  // still parses. They stay DELIBERATELY absent from
  // `SELFHOST_EVIDENCE_CATEGORIES` below (the green-gating deletion booleans
  // every self-host scenario shares).
  | "e2b_sandbox"
  | "e2b_template"
>;
