...
function failedResult(overrides: Partial<FinalCellResultV1> = {}): FinalCellResultV1 {
  return {
    ...
    registry_flow_ref: "tests/release/core-release-scenario-manifest.json#T3-EXAMPLE",
    runtime_lane: "local",
    dimensions: {},
    status: "failed",
...
test("toFailureReports maps a normalized failed result to the issue payload", () => {
  ...
  assert.equal(report.flow, "tests/release/core-release-scenario-manifest.json#T3-EXAMPLE");
