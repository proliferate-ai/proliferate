/**
 * Strict Tier-3/Tier-4 release-policy machinery.
 *
 ...
 *
 * This module is pure and lane-agnostic. The CLI derives the executable
 * manifest from the complete registered scenario set, then compares collected
 * rows against it. The separate target inventory in
 * `specs/developing/testing/core-release-scenario-manifest.json` is deliberately
 * not treated as executable coverage until a row has a concrete collector,
 * lane, gate, and evidence state.
 */
...
/** The executable required-scenario manifest derived from the runner registry. */
 ...
 * at once; `buildRequiredManifest`/`evaluate` throw `ManifestConfigError` on a
 * non-empty result.
 */
...
/** Builds and validates an executable manifest from concrete registry rows. */
export function buildRequiredManifest(rows: readonly RequiredRow[]): RequiredManifest {
  const manifest: RequiredManifest = {
    version: 1,
    required: rows.map(({ id, lane }) => ({ id, lane })),
  };
 ...
 *
 * Emitted result keys outside the executable manifest are also evaluated: an
