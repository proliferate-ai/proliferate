/**
 * Release-policy unit coverage:
 *   pnpm -C tests/release exec tsx --test src/runner/release-policy.test.ts
 *
...
// A small, self-contained executable manifest.
const MANIFEST: RequiredManifest = {
  version: 1,
  required: [
    { id: "T3-CHAT-1", lane: "cloud" },
    { id: "T3-PROV-1", lane: "desktop" },
  ],
};
...
function allGreen(): ResultRow[] {
  return [
    ...
    { id: "T3-PROV-1", lane: "desktop", status: "green" },
  ];
}
...
test("release mode fails on a MISSING required row", () => {
  const results: ResultRow[] = [{ id: "T3-CHAT-1", lane: "cloud", status: "green" }];
...
for (const status of ["blocked", "skipped", "expected-fail", "cancelled", "failed"] as const) {
  test(`release mode fails on a ${status} required row`, () => {
    const results: ResultRow[] = [
      { id: "T3-CHAT-1", lane: "cloud", status: status as ScenarioStatus },
      { id: "T3-PROV-1", lane: "desktop", status: "green" },
    ];
    const evalResult = evaluate(MANIFEST, results, "release");
    assert.equal(evalResult.ok, false, `${status} must fail release mode`);
    assert.equal(evalResult.exitCode, 1);
    assert.match(evalResult.violations.join("\n"), new RegExp(`T3-CHAT-1/cloud: ${status}`));
  });
}
...
test("release mode rejects a non-green emitted result outside the executable manifest", () => {
  const results: ResultRow[] = [
    ...allGreen(),
...
test("release mode rejects duplicate emitted results outside the executable manifest", () => {
  const results: ResultRow[] = [
    ...allGreen(),
    { id: "T3-BILL-2", lane: "cloud", status: "green" },
    { id: "T3-BILL-2", lane: "cloud", status: "green" },
  ];
  const evalResult = evaluate(MANIFEST, results, "release");
  assert.equal(evalResult.ok, false);
  assert.equal(evalResult.counters.duplicate, 1);
  assert.match(evalResult.violations.join("\n"), /T3-BILL-2\/cloud: duplicate/);
});
...
test("signal mode preserves current permissive behavior (blocked/expected-fail/missing pass)", () => {
  const results: ResultRow[] = [
    { id: "T3-CHAT-1", lane: "cloud", status: "blocked" },
    // T3-PROV-1/desktop missing entirely
  ];
...
test("signal mode is informational even when a required row failed", () => {
  const results: ResultRow[] = [
    { id: "T3-CHAT-1", lane: "cloud", status: "failed" },
    { id: "T3-PROV-1", lane: "desktop", status: "green" },
  ];
  const evalResult = evaluate(MANIFEST, results, "signal");
  assert.equal(evalResult.ok, true);
...
test("validateManifest flags a duplicate (id, lane) pair as a config error", () => {
  ...
  assert.match(errors[0], /duplicate required row T3-CHAT-1\/cloud/);
});
...
test("buildRequiredManifest copies and validates concrete registry rows", () => {
  const manifest = buildRequiredManifest(MANIFEST.required);
  assert.deepEqual(validateManifest(manifest), []);
  assert.deepEqual(manifest, MANIFEST);
  assert.throws(
    () => buildRequiredManifest([...MANIFEST.required, MANIFEST.required[0]]),
    ManifestConfigError,
  );
});
...
test("buildSummary matches the plan's field shape and passes release validation when fully populated", () => {
  ...
  assert.deepEqual(summary.required, ["T3-CHAT-1/cloud", "T3-PROV-1/desktop"]);
...
test("signal validation ignores unknown identity fields and non-green counters", () => {
  const results: ResultRow[] = [{ id: "T3-CHAT-1", lane: "cloud", status: "blocked" }];
  const evalResult = evaluate(MANIFEST, results, "signal");
  const summary = buildSummary({
    policy: "signal",
    target: "local",
...
test("scenarioCorrelationId is unique per run", () => {
  const a = scenarioCorrelationId("T3-CHAT-1", "cloud");
  const b = scenarioCorrelationId("T3-CHAT-1", "cloud");
  assert.notEqual(a, b);
  assert.match(a, /^T3-CHAT-1\/cloud\//);
});
...
test("the runner does not retry after an external effect (structural invariant)", () => {
  ...
  guard.begin("T3-CHAT-1", "cloud");
  guard.begin("T3-CHAT-1", "desktop"); // distinct lane is fine
  assert.throws(() => guard.begin("T3-CHAT-1", "cloud"), /Refusing to retry/);
});
...
test("withDeadline rejects work that exceeds the deadline and resolves fast work", async () => {
  ...
  await assert.rejects(
    withDeadline(new Promise((resolve) => setTimeout(resolve, 50)), 5, "T3-CHAT-1/cloud"),
    /exceeded the 5ms scenario deadline/,
  );
  const value = await withDeadline(Promise.resolve("ok"), SCENARIO_DEADLINE_MS, "T3-CHAT-1/cloud");
