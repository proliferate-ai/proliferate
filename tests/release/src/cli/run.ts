...
/**
 * Maps the runner's runtime vocabulary onto stable evidence lanes. Sandbox
 * scenarios exercise the cloud target; local runtime scenarios exercise the
...
const RELEASE_SUMMARY_FILENAME = "release-qualification-summary.json";
...
async function main(): Promise<void> {
  ...
  // Result rows in the stable evidence-lane vocabulary.
  const results: ResultRow[] = [];
  // Structural no-retry-after-external-effect guard: each scenario/lane
  ...
  for (const scenario of scenarios) {
    for (const runtimeLane of scenario.lanes) {
      ...
      // Live-scenario policy: each scenario/lane runs exactly once (the
  ...
  // In `release` policy the complete executable scenario registry
...
/**
 * Release gate. Derives the required rows from the complete executable
 * scenario registry, evaluates the collected results strictly, emits the
 * summary artifact, and sets a
...
async function runReleaseGate(
  ...
  const manifest = buildRequiredManifest(
    selectScenarios("all").flatMap((scenario) =>
      scenario.lanes.map((runtimeLane) => ({
        id: scenario.id,
        lane: runtimeLaneToReleaseLane(runtimeLane),
      })),
    ),
  );
  ...
  const summaryPath = path.join(args.outputDir, RELEASE_SUMMARY_FILENAME);
