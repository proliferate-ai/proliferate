 ...
 * specs/TESTING/scenarios.md#T3-REPO-1
 *
...
export const t3Repo1: ScenarioDefinition = {
  ...
  registryFlowRef: "specs/TESTING/scenarios.md#T3-REPO-1",
  lanes: ["local", "sandbox"],
  ...
  run: async (ctx) => {
    ...
    if (ctx.runtimeLane === "local" && isWorldBackedRun(ctx)) {
      await runLocal1WorkspaceLeaf(ctx, {
        ...
        registryFlowRef: "specs/TESTING/tier-3-scenario-contract.md#local-1",
      });
      return;
    }
    await runReal();
