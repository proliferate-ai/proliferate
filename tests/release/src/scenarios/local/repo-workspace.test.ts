...
function cell(scenarioId: string): PlannedCellV1 {
  return {
    ...
    registry_flow_ref: "delivery/testing-cicd/archive/tier-3-scenario-contract.md#local-1",
    runtime_lane: "local",
    dimensions: {},
    required_env: [],
  };
...
test("runLocal1WorkspaceLeaf: green outcome resolves without throwing", async () => {
  ...
  await runLocal1WorkspaceLeaf(
    fakeCtx(),
    { scenarioId: "T3-REPO-1", registryFlowRef: "delivery/testing-cicd/archive/tier-3-scenario-contract.md#local-1" },
    driver,
  );
});
