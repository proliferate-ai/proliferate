/**
 * Secure local credential loading for the local-runtime world.
 *
 * Ported (selectively) from the combined foundation worktree's
 * `config/local-environment.ts`: the credential file is parsed as DATA, never
 * sourced as a shell, and ambient environment always wins so an operator can
 * override one value without editing the shared file. Refuses symlinks and
 * world/group-readable files, and only materializes keys declared in the
 * release runner's env manifest.
 *
 * Additional provider credentials may live in `server/.env`; those are parsed
 * the same way (as data) and only names ever reach evidence.
 */
...
const ALLOWED_FILE_KEYS = new Set(envVarNames());

export interface ReleaseEnvironmentLoadResult {
  status: "loaded" | "missing" | "skipped-ci";
  filePath: string;
  explicitFile: boolean;
  /** Names copied into env this call (were absent from the ambient env). */
  loadedNames: readonly string[];
  /** Names left untouched because the ambient env already had them. */
...
/**
 * Load the local release runner's credential file as data. CI never reads the
 * default file (only an explicit `RELEASE_E2E_ENV_FILE`).
 */
export function loadReleaseEnvironment(
  ...
  if (!explicitFile && isCi(env)) {
    return emptyResult("skipped-ci", filePath, explicitFile);
  }
  ...
  try {
    ...
    if (isMissingFileError(error)) {
      ...
      return emptyResult("missing", filePath, explicitFile);
    }
    throw error;
  }
...
export function parseReleaseEnvironmentFile(
  contents: string,
  source = "release-e2e.env",
): Map<string, string> {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!match) {
      throw new ReleaseEnvironmentFileError(
        `${source}:${index + 1}: expected NAME=value or export NAME=value`,
      );
    }
    const name = match[1];
    if (parsed.has(name)) {
...
function emptyResult(
  status: ReleaseEnvironmentLoadResult["status"],
  filePath: string,
  explicitFile: boolean,
): ReleaseEnvironmentLoadResult {
  return { status, filePath, explicitFile, loadedNames: [], preservedNames: [], ignoredNames: [] };
}
