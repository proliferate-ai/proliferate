...
- `signal` is the CLI/local default. Blocked and expected-fail rows remain
  ...
  fails the run.
- `release` is the manual GitHub Actions default. Every scenario/lane in the
  complete executable runner registry must be present exactly once and green.
  Missing credentials, blocked or expected-fail results, duplicate results,
  partial `--scenarios` selection, and missing registered rows therefore fail
  strict qualification.

Neither policy proves the complete product contract yet: the target manifest
keeps unaudited rows explicitly `planned`, and the executable registry remains
smaller than `specs/developing/testing/core-release-validation.md`.
...
- **Rotation.** Staging refresh tokens rotate on EVERY use. Bootstrap once with
  ...
  `RELEASE_E2E_STAGING_SESSION_STATE` overrides). In CI plaintext exists only
  in the ephemeral job workspace. Before `actions/cache` persistence the state
  is sealed with AES-256-GCM using the dedicated, stable
...
until every emitted result is green and every required executable row is
present. The required set is the complete executable runner registry, not the
larger target inventory whose unaudited rows remain explicitly planned.
