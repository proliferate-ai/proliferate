...
`docs/primitives/` and `docs/features/`; keep this file only until its glossary
and dependency-order notes have been folded into the owning docs.

Date: 2026-05-20.

Use `docs/primitives/sandbox-provisioning.md` as the foundation. Every other
cloud, sandbox, auth, command, or dispatch doc assumes that primitive.

## Files

```text
docs/primitives/sandbox-provisioning.md   Cloud target / managed sandbox foundation.
docs/primitives/mcp-skills.md             Sandbox-scoped MCP/skill/plugin runtime config.
docs/primitives/agent-auth.md             Sandbox-scoped agent LLM auth and gateway.
docs/features/settings-admin-ia.md        Settings/Admin information architecture and shared UI primitives.
docs/primitives/cloud-commands.md         Cloud command queue, worker dispatch, projection, preflight.
docs/primitives/claiming.md               Shared unclaimed work, claim ownership, Desktop direct-attach tokens.
docs/features/automations.md              Scheduled/triggered work that reuses sandbox/auth/command primitives.
docs/features/slack-bot.md                Slack bot as a team-automation entrypoint.
docs/features/cloud-dispatch.md           Cloud-mediated web/mobile clients and remote-access UX.
docs/primitives/billing.md                Entitlement, compute usage, managed credits, runtime blocks.
docs/features/workspace-migration.md      Move runnable workspace/session state between targets.
```
