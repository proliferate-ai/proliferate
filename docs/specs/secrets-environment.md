...
- [ ] **Snapshot scrub/re-apply does not yet cover `secretFileWrites`** — snapshot scrub currently targets `configurations.envFiles` spec only. Impact: file-based secrets materialized from `secret_files` may persist in snapshots until scrub parity is added. Tracking: `TODO(secretfilewrites-snapshot-scrub-parity)`.
