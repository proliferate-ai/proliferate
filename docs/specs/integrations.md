...
- [ ] **User-scoped credential resolution is not implemented.** Authored schema only persists org-scoped integrations and connectors, and `getToken()` has no user-attribution branch yet. This blocks first-class user-authored external actions. Evidence: `packages/db/src/schema/integrations.ts`, `packages/services/src/integrations/tokens.ts`.
