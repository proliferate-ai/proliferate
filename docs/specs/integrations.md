...
- OAuth start must mint base64url JSON state containing org/user context + nonce + timestamp + optional return URL, signed with server-side HMAC.
- Callback must reject missing, unsigned, tampered, or expired OAuth state before trusting any state fields.
- Callback must re-validate that the authenticated user is a member of the state org before persistence.
...
- OAuth callback must re-validate that the authenticated callback user matches state user and is still a member of the state org.
