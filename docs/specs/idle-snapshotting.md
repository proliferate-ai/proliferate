...
3. **Hub accumulation.** Not evicting hubs avoids races, but hubs accumulate in `hub-manager.ts`. Stopping `setInterval` after snapshot prevents CPU leaks. Memory growth from idle hub objects is proportional to total sessions on this gateway instance — acceptable for v1, but worth tracking. A separate cleanup pass (e.g., evict hubs whose sessions have `endedAt` older than N hours) can be added later.
