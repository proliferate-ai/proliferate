...
1. **Don't evict hub** — hub stays in `HubManager` after idle snapshot; reused on resume.
   - Without this: `getOrCreate()` deduplicates by session ID via a pending-promise map. If we evict the hub, a reconnecting client calls `getOrCreate()` which creates a new hub — but the old hub may still have in-flight requests (e.g. a prompt blocked on the migration lock). Two hubs for the same session means split state and duplicate SSE connections.
...
8. **`removeProxy()` must be idempotent** — the cleanup closure uses a `removed` flag and `Math.max(0, ...)` to prevent `proxyConnections` going negative.
   - Without this: if the terminal/VS Code WS `close` handler fires twice (e.g. error then close), or if an early error path calls cleanup before the normal close, `proxyConnections` decrements below 0. A negative count means `proxyConnections > 0` is never true, so the idle check's `this.proxyConnections > 0` guard never blocks a snapshot — but the count is wrong, potentially masking real connections.
...
12. **Cancel pending reconnect before idle snapshot** — cancel the `scheduleReconnect()` timer and use a generation counter to invalidate in-flight callbacks.
    - Without this: automation sessions have `shouldReconnectWithoutClients() === true`. When SSE drops, `handleSseDisconnect()` calls `scheduleReconnect()`, which sets a `setTimeout` to call `ensureRuntimeReady()`. If idle snapshot fires during that delay window, it snapshots and terminates the sandbox. But the pending timer callback still fires, calls `ensureRuntimeReady()`, restores the sandbox from the snapshot, and now the sandbox is running again with 0 clients — reintroducing the idle burn. The generation counter handles the subtler race where the timer callback has already fired and is blocked on the migration lock inside `ensureRuntimeReady()`.
...
2. **No proxy connections** — `proxyConnections === 0` (terminal/VS Code WS proxies)
...
`mem:` prefix distinguishes memory snapshot IDs from filesystem image IDs. Only the Modal provider interprets this prefix.
...
runIdleSnapshot():
  ...
  6. Snapshot:
     ...
     — Modal: cpClient.sandboxSnapshot() → mem: prefixed ID
     — Fallback: provider.snapshot() (filesystem) if memory snapshot fails
  7. Terminate (non-pause providers only): provider.terminate()
  8. Update DB:
     — snapshotId, sandboxId: null, status: "paused", pausedAt: now(), pauseReason: "inactivity"
...
Web client opens session → addClient() → initializeClient()
  → ensureRuntimeReady()
    ...
    → provider.ensureSandbox({ snapshotId })
      ...
      → For filesystem snapshots: Sandbox.create(image=snapshot) → new sandbox + boot sequence
...
No `setupSandbox()`, `setupEssentialDependencies()`, or `waitForOpenCodeReady()` — processes are already running. However, git freshness must be handled post-thaw (see section 1g).
...
#### 1g. Post-thaw git freshness

Memory snapshot restore skips `setupSandbox()` / `setupAdditionalDependencies()`, which means the Phase 2 git freshness pull (`shouldPullOnRestore()` → `git pull --ff-only`) never runs. A user who pauses Friday and resumes Monday gets stale git state.

After `restoreFromMemorySnapshot()` succeeds, fire-and-forget a git freshness check using the existing `shouldPullOnRestore()` gate and `execCommand()` against the restored sandbox:

```ts
// In ensureSandbox(), after successful memory restore:
if (shouldPullOnRestore(opts)) {
  // Fire-and-forget — don't block resume
  this.runGitFreshnessPull(newSandboxId, opts, log).catch((err) => {
    log.warn({ err }, "Post-thaw git freshness pull failed");
  });
}
```

This reuses the existing cadence gating (`SANDBOX_GIT_PULL_ON_RESTORE`, `SANDBOX_GIT_PULL_CADENCE_SECONDS`) and credential re-write logic from `setupAdditionalDependencies()`. The pull is non-blocking — the user can start working immediately while git pulls in the background.
...
addProxyConnection(): () => void {
  ...
  return () => {
    ...
    removed = true;
    this.proxyConnections = Math.max(0, this.proxyConnections - 1);
    this.touchActivity();
  };
}
...
`removeProxy()` uses a closure flag + `Math.max(0, ...)` to prevent underflow from early error paths.
...
private checkIdleSnapshot(): void {
  ...
  if (this.clients.size > 0 || this.proxyConnections > 0) return;
...
#### 3i. Store reconnect timer + generation counter for safe cancellation

The existing `scheduleReconnect()` uses a bare `setTimeout`. Store the timer ID and a generation counter so idle snapshot can cancel it — including the case where the timer callback already fired but is blocked on the migration lock:
...
The `reconnectGeneration` counter handles the race where the timeout callback has already fired and is waiting on `ensureRuntimeReady()` → migration lock. When `cancelReconnect()` increments the generation, the callback sees a mismatch and aborts.
  ...
  const ran = await runWithMigrationLock(this.options.sessionId, 300_000, async () => {
    ...
    if (provider.supportsPause) {
      ...
      try {
        ...
        this.logger.warn({ err }, "Memory snapshot failed, falling back to filesystem");
    ...
    // 5. DB update — status "paused", no endedAt, no markSessionStopped
    await sessions.update(this.options.sessionId, {
      snapshotId,
      sandboxId: provider.supportsPause ? freshSandboxId : null,
      status: "paused",
...
**Memory snapshot failure at snapshot time**: fallback to filesystem snapshot is acceptable. The sandbox is still running, so the filesystem snapshot captures current state including OpenCode's data directory on disk. Resume will be slower (~10-20s) and dev servers won't be running, but the OpenCode transcript is preserved.
...
| Reconnect cancellation (generation counter) | **New** (prevents automation reconnect race) |
...
5. **Hub reuse**: After idle snapshot, hub stays in HubManager. Reconnecting client gets the same hub.
...
1. **Preview/SSH "activity" is invisible to the gateway.** Modal `previewUrl` and SSH tunnels are direct-to-Modal URLs, not routed through Gateway. `clients.size`/`proxyConnections` tracking cannot see a user actively using preview or SSH in another tab/tool. The gateway may snapshot underneath them. Preview URLs come back on resume; SSH connections must reconnect.
...
3. **Hub accumulation → vNext eviction.** Not evicting hubs avoids races in the current single-instance architecture, but hubs accumulate in `hub-manager.ts`. Stopping `setInterval` after snapshot prevents CPU leaks. Memory growth from idle hub objects is proportional to total sessions on this gateway instance — acceptable for v1. When `vNext/sessions-gateway.md` Session Ownership Leases ship (`SET owner:{sessionId} {instanceId} NX PX 30000`), idle snapshot should evict the hub and release the lease instead of keeping it alive. The lease mechanism prevents split-brain, making eviction safe. Until then, a separate cleanup pass (e.g., evict hubs whose sessions have `endedAt` older than N hours) can be added as a stopgap.
...
6. **vNext tool callbacks and false idle.** Currently, tools are intercepted via SSE events (`event-processor.ts`), so `getCurrentAssistantMessageId()` stays non-null throughout tool execution. If `vNext/agent-contract.md` moves tool execution to synchronous HTTP callbacks (`POST /internal/tools/:toolName`), verify that OpenCode still emits SSE events that keep the assistant message active during tool execution. If not, an `activeHttpToolCalls` counter in `SessionHub` would be needed to prevent idle snapshot during long-running tool calls.

7. **Automation fast-path after `automation.complete`.** When an automation agent calls `automation.complete`, the run is definitively finished. The 30s automation grace still burns compute waiting for the agent to fully idle. A future optimization: once `automation.complete` has been called, reduce or eliminate the grace period. Immediate termination is risky (agent may still be generating text after the tool returns), but a ~5s grace would reclaim most of the waste.
...
**Cons:**
- Memory snapshots are pre-beta — stability risk (mitigated by filesystem snapshot fallback)
