...
| 6 | `actions.md` | Action invocations, approval flow, mode resolution, risk classification, provider adapters (Linear/Sentry/Slack/Jira), sweeper. | 2 |
...
| 10 | `secrets-environment.md` | Secret CRUD, secret files, bulk import, env file deployment to sandbox, encryption. | 3 |
...
### Supplemental docs (non-canonical)

- `docs/specs/agent-platform-v1/implementation/progress.md` is a historical implementation log for the V1 rollout.
- It is **not** a canonical ownership spec and must not be used as the source of truth for current runtime contracts.
- When it disagrees with `main`, update canonical specs and feature registry to match code on `main`.
...
| **Triggers vs Automations** | `triggers.md` owns event ingestion, matching, and dispatch. Once a trigger fires, the resulting automation run belongs to `automations-runs.md`. The handoff point is the `enqueue_enrich` outbox row (inserted transactionally with run + event); the outbox poller dispatches to `AUTOMATION_ENRICH` BullMQ downstream. |
