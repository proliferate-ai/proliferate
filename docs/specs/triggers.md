...
- ~~Assuming trigger list pending counts represent queued work.~~ Previously misaligned (`pending` vs `queued`); now fixed — `getPendingEventCounts` queries `status = 'queued'` (`packages/services/src/triggers/db.ts`).
...
- [x] **Pending count status bug (Medium):** Resolved — `getPendingEventCounts` now queries `status = 'queued'`, matching the canonical trigger event lifecycle. Evidence: `packages/services/src/triggers/db.ts:getPendingEventCounts`, `packages/db/src/schema/triggers.ts`.
