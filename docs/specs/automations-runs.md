...
- Template instantiation in coworker UI is worker-first: `automations.createFromTemplate` now returns `{ worker }` and routes to coworker detail, not legacy automation detail (`apps/web/src/server/routers/automations.ts`, `apps/web/src/hooks/org/use-templates.ts`).
