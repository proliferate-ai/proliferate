...
(spec §3 flow 2's "Verification is the ordinary launch probe").

What IS here is both halves of spec §3 flow 5. Slice 2 adds the **limit-hit
intake** — the courier's relay of a runtime-observed limit error, the hard
signal and the audit half of rotation (the rotation *decision* stays
runtime-local). Slice 4 adds the **usage probe** — the soft signal (meters): a
one-token request per active seat whose rate-limit headers feed
``seat_usage_sample`` and the settings meters. Advisory only, never a launch
gate: nothing in the launch/render path reads samples (import-scan enforced),
and a probe failure changes no launch behavior.
"""
