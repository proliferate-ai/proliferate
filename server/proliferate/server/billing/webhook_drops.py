...
grant, a hold, or a subscription projection. Those early returns used to be
completely silent while the receipt was still marked ``processed``: Stripe
never retried, and nothing recorded that money had arrived without landing
anywhere (W-F5). Every early return now reports through here, which owns both
the stable drop vocabulary and the per-drop context each reason carries.
...
def report_checkout_unhandled_purpose(
    ...
    *,
    ...
    """Subscription-mode completions are entitled by ``invoice.paid``, and other
    purposes have no local projection, so this arm is ordinary traffic."""
    report_drop(
...
def report_invoice_grant_gate_closed(
    ...
    With Pro pricing off that is the ruled legacy shape; with it on, the missing
    projection is why the customer sees no hours, so the closed gate is broken
    down into the four conditions that can shut it.
    """
    pro_pricing_enabled = settings.pro_billing_enabled
