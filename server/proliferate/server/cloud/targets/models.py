...
class TargetStatusPayload(BaseModel):
    ...
    safe_stop_reasons: list[object] = Field(serialization_alias="safeStopReasons")
...
class TargetInventoryPayload(BaseModel):
    ...
    shell: str | None = None
    package_managers: list[object] = Field(default_factory=list, serialization_alias="packageManagers")
    workspace_roots: list[object] = Field(default_factory=list, serialization_alias="workspaceRoots")
...
def target_status_payload(status: TargetStatusRecord | None) -> TargetStatusPayload | None:
    if status is None:
        return None
...
def target_inventory_payload(
    inventory: TargetInventoryRecord | None,
...
def target_summary_payload(
    target: TargetRecord,
    *,
    status: TargetStatusRecord | None,
    inventory: TargetInventoryRecord | None,
) -> TargetSummary:
