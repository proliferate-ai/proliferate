...
from proliferate.server.cloud.mobility.models import WorkspaceMobilityPreflightResponse
...
async def list_cloud_workspace_mobility_for_user(user_id: UUID):
...
async def preflight_cloud_workspace_handoff(
    *,
    ...
    excluded_paths = (
        [item.relative_path for item in repo_config.tracked_files] if repo_config is not None else []
    )
