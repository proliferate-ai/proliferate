...
async def get_launch_options_endpoint(
    harness_kind: str,
    sandbox: LaunchOptionsSandboxAccess,
    db: AsyncSession = Depends(get_async_session),
) -> CopiedLaunchOptionsResponse:
    return await get_launch_options(
        db,
        sandbox=sandbox,
        harness_kind=harness_kind,
    )
