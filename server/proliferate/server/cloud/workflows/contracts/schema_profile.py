...
def _validate_type(node_type: object) -> None:
...
def _validate_node(node: object, *, is_root: bool) -> None:
    ...
    for numeric_key in ("minimum", "maximum"):
        if numeric_key in node:
            bound = node[numeric_key]
            if (
...
def validate_schema_profile(document: object) -> None:
