...
async def worker_heartbeat_endpoint(
    ...
    try:
        ...
        return await record_heartbeat(db, worker_id=worker_id, target_id=target_id, body=body)
    except CloudApiError as error:
        raise_cloud_error(error)
...
async def lease_worker_command_endpoint(
    ...
    worker_id, target_id, _ = identity
    try:
        return await lease_worker_commands(
            db,
            target_id=target_id,
            worker_id=worker_id,
...
async def worker_command_delivery_endpoint(
    ...
    try:
        worker_id, _, _ = identity
        await record_command_delivery(db, command_id=command_id, worker_id=worker_id, body=body)
    except CloudApiError as error:
        raise_cloud_error(error)
    return {"ok": True}
...
async def worker_command_result_endpoint(
    ...
    try:
        worker_id, _, _ = identity
        await record_command_result(db, command_id=command_id, worker_id=worker_id, body=body)
    except CloudApiError as error:
        raise_cloud_error(error)
    return {"ok": True}
...
async def worker_update_status_endpoint(
    _: WorkerUpdateStatusRequest,
    __: tuple[UUID, UUID, UUID] = Depends(_worker_identity),
) -> WorkerUpdateStatusResponse:
    return WorkerUpdateStatusResponse(accepted=True)
