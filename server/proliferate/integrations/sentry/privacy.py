...
_ANGLE_NAMES = frozenset(_words("<module> <locals> <lambda> <genexpr> <listcomp> <dictcomp> "
                                "<setcomp>"))
...
def _qualname_ok(cleaned: str) -> bool:
    if any(ch in cleaned for ch in ("/", "\\", ":")) or re.search(r"\s", cleaned):
        return False
    return all(
        seg in _ANGLE_NAMES or bool(_PY_NAME.match(seg)) for seg in cleaned.split(".")
    )
...
BREADCRUMB_TYPES = frozenset(_words("default debug error http info navigation query "
                                    "transaction ui user log"))
SPAN_OPS = frozenset(_words("http.server websocket.server queue.task.celery queue.process "
                            "queue.publish"))
SPAN_STATUSES = frozenset(_words(
...
_LABEL_PREFIXES = _words("materialize_sandbox materialize_repo_environment "
...
_UUID_TAGS = _words("user_id organization_id cloud_workspace_id cloud_target_id "
...
TAG_VALIDATORS: dict[str, Any] = {
    **{key: canonical_uuid for key in _UUID_TAGS},
    ...
    "domain": _catalog("anonymous_telemetry billing agent_gateway cloud_materialization "
                       "cloud_materialization_failure_persistence", 41),
    "action": _catalog("heartbeat_start heartbeat_loop authorization_read reconcile_loop "
                       "stripe_webhook_drop verification enrollment_backfill usage_import "
                       "llm_topup", 19),
    "harness_kind": _catalog("claude codex opencode grok", 8),
    "label": _label,
    "fn": _catalog("_repair_materialize_sandbox materialize_sandbox materialize_repo_environment "
                   "materialize_secret_set materialize_agent_auth", 28),
}

_UUID_EXTRAS = _words("billing_subject_id owner_user_id subject_id")
_BOOL_EXTRAS = _words("subject_resolved pro_pricing_enabled has_subscription_record "
...
EXTRA_VALIDATORS: dict[str, Any] = {
    ...
    **{key: _bool for key in _BOOL_EXTRAS},
    ...
    "invoice_reason": _catalog("subscription_update subscription_threshold manual upcoming "
                               "quote_accept automatic_pending_invoice_item_invoice", 44),
