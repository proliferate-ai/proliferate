...
async def get_runtime_config_status(
    runtime_url: str,
    access_token: str,
) -> dict[str, object]:
    try:
        ...
        raise CloudRuntimeReconnectError("Failed to load cloud runtime config status.") from exc
    if not response.is_success:
        raise CloudRuntimeReconnectError(
            rejected_response_message(
                "load cloud runtime config status", response.status_code, response.text
            )
        )
    try:
        payload: object = response.json()
        ...
        raise CloudRuntimeReconnectError(
            "Cloud runtime returned invalid JSON for runtime config status."
        ) from exc
    if not isinstance(payload, dict):
        raise CloudRuntimeReconnectError(
            "Cloud runtime returned an invalid runtime config status response."
        )
    return payload
