...
async def _token_request(token_endpoint: str, data: dict[str, str]) -> TokenResponse:
    async with httpx.AsyncClient(timeout=20.0) as client:
        response = await client.post(token_endpoint, data=data)
        text = response.text
        if response.status_code in {400, 401} and "invalid_client" in text:
            raise McpOAuthProviderError("invalid_client", "OAuth provider rejected client.")
        if response.status_code == 400 and "invalid_grant" in text:
