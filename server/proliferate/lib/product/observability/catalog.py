...
P0_OPERATION_FAMILIES: Final = (
    OperationFamilySpecV1(
        ...
        boundary=(
            "Collector initialization, shutdown, export, or registration begins through "
            "readiness, orderly stop, snapshot completion, or admission/rejection."
        ),
        owning_pr=2,
    ),
    OperationFamilySpecV1(
        pattern=(
            "desktop.collector.{start,restart,stop}; "
            "desktop.anyharness_process.{start,restart,stop}; "
            "desktop.worker_process.{start,stop}"
        ),
        operations=(
        ...
        boundary=(
            "The supervisor accepts the process action through child readiness, stop, "
            "or an allowed terminal."
        ),
        owning_pr=3,
    ),
    OperationFamilySpecV1(
        ...
        boundary=(
            "Intent acceptance or boot start through client orchestration completion, "
            "prompt handoff, or an allowed terminal."
        ),
        owning_pr=8,
    ),
    OperationFamilySpecV1(
        ...
        boundary=(
            "After per-export consent, preparation or submission begins through bounded "
            "artifact or receipt, or an allowed terminal."
        ),
        owning_pr=6,
    ),
    OperationFamilySpecV1(
        ...
        boundary=(
            "Work admission through semantic or protocol result, including observable "
            "unwind, transport loss, or an allowed terminal."
        ),
        owning_pr=9,
    ),
    OperationFamilySpecV1(
        ...
        boundary=(
            "Initialization, shutdown, connection, or command admission through readiness, "
            "stop, connection, result, or an allowed terminal."
        ),
        owning_pr=9,
    ),
    OperationFamilySpecV1(
        ...
        boundary=(
            "Transport admission through response completion, close, forward resolution, "
            "or an allowed transport terminal; never domain work."
        ),
        owning_pr=10,
    ),
    OperationFamilySpecV1(
        ...
        semantic_owner=(
            "Owning server domain service; model gateway only when it constructs and owns "
            "the provider request"
        ),
        boundary=(
            "Domain work admission through committed or known domain result, including "
            "worker loss or an allowed terminal."
        ),
        owning_pr=11,
    ),
)

P0_OPERATION_NAMES: Final = tuple(
    operation for family in P0_OPERATION_FAMILIES for operation in family.operations
)
_P0_OPERATION_NAME_SET: Final = frozenset(P0_OPERATION_NAMES)


def is_p0_operation(name: str) -> bool:
    return name in _P0_OPERATION_NAME_SET
