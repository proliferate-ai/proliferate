...
if TYPE_CHECKING:
    from proliferate.server.cloud.integrations.seeds import SeedDefinition
...
async def upsert_seed_definition(
    db: AsyncSession, spec: SeedDefinition
) -> IntegrationDefinitionRecord:
    """Insert or update the seed definition for ``spec.namespace``.

    Matches on ``source='seed'`` + ``namespace``. Updates the mutable seed
    fields; never touches org-custom rows. Idempotent.
    """
    config_json = serialize_definition_config(spec.config)
    row = await db.scalar(
        select(CloudIntegrationDefinition).where(
            CloudIntegrationDefinition.source == "seed",
            CloudIntegrationDefinition.namespace == spec.namespace,
        )
    )
    if row is None:
        row = CloudIntegrationDefinition(
        ...
        row.auth_kind = spec.auth_kind
        row.oauth_client_mode = spec.oauth_client_mode
        row.config_json = config_json
        row.enabled_by_default = spec.enabled_by_default
