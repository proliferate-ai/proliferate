...
async def legacy_delete_connection(
    db: AsyncSession,
    *,
    user_id: UUID,
    connection_id: str,
) -> None:
    await delete_user_connection(db, user_id, connection_id)


async def legacy_list_connections(
    db: AsyncSession,
    user_id: UUID,
) -> list[CloudMcpConnectionRecord]:
    return await list_user_connections(db, user_id)
