...
copy_unmanaged_env_values() {
  ...
  managed_pattern="^(POSTGRES_PASSWORD|DATABASE_URL|JWT_SECRET|CLOUD_SECRET_KEY|LITELLM_POSTGRES_PASSWORD|LITELLM_MASTER_KEY|AGENT_GATEWAY_LITELLM_MASTER_KEY|SITE_ADDRESS|API_BASE_URL|PROLIFERATE_PUBLIC_HEALTHCHECK_URL|PROLIFERATE_USE_SSLIP_FALLBACK)$"
...
LITELLM_MASTER_KEY="$(resolve_value \
  ...
  32)"
# API service needs AGENT_GATEWAY_LITELLM_MASTER_KEY to auth with the LiteLLM proxy.
# Set it to the same value as LITELLM_MASTER_KEY.
AGENT_GATEWAY_LITELLM_MASTER_KEY="$LITELLM_MASTER_KEY"
