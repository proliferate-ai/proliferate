...
@pytest.mark.asyncio
async def test_provision_sso_jit_membership_uses_owner_order(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db = cast(AsyncSession, object())
    organization_id = uuid4()
    actor_user = SimpleNamespace(id=uuid4(), email="person@example.com", display_name=None)
    membership = _accepted_record(
        organization_id=organization_id,
        membership_id=uuid4(),
    ...
    async def guard(call_db: AsyncSession, **kwargs: object) -> None:
        assert call_db is db
        assert kwargs == {
            "organization_id": organization_id,
            "user_id": actor_user.id,
    ...
    async def ensure(call_db: AsyncSession, **kwargs: object) -> MembershipRecord:
        assert call_db is db
        assert kwargs == {
            "organization_id": organization_id,
            "user_id": actor_user.id,
    ...
    async def seat(call_db: AsyncSession, **kwargs: object) -> None:
        assert call_db is db
        assert kwargs == {"organization_id": organization_id, "membership_id": membership.id}
        events.append("seat")
    ...
    def enrollment(
        ...
        *,
        ...
        assert (call_organization_id, call_user_id, db) == (
            organization_id,
            actor_user.id,
            expected_db,
        )
    ...
    expected_db = db
    monkeypatch.setattr(organization_service, "ensure_instance_membership_not_removed", guard)
    monkeypatch.setattr(
        organization_service.organization_store, "ensure_sso_jit_membership", ensure
    )
    monkeypatch.setattr(organization_service, "maybe_create_organization_seat_adjustment", seat)
    monkeypatch.setattr(organization_service, "schedule_agent_gateway_org_enrollment", enrollment)
    ...
    assert events == ["guard", "membership", "seat", "enrollment"]
