...
SENTINELS = [
    ...
    "PROMPT_SENTINEL_do_not_ship",
    "TRANSCRIPT_SENTINEL_do_not_ship",
    "TERMINAL_OUTPUT_SENTINEL_do_not_ship",
    "FILE_CONTENT_SENTINEL_do_not_ship",
    "/Users/private/acme-secret-repo/private.py",
    "REQUEST_BODY_SENTINEL_do_not_ship",
    "QUERY_SENTINEL_do_not_ship",
    "HEADER_SENTINEL_do_not_ship",
    "ENV_VALUE_SENTINEL_do_not_ship",
    "PROVIDER_RESPONSE_SENTINEL_do_not_ship",
...
# --- identity, timing, grouping ----------------------------------------------


def test_positive_identity_and_exception_shape_survives_byte_for_byte() -> None:
    entry = {
        "type": "Outer.InnerError",
        "module": "proliferate.server.billing.errors",
        "value": SENTINELS[11],
        "notes": [SENTINELS[5]],
        ...
        "stacktrace": {
            "frames": [
                ...
                },
                {
                    ...
                    "function": "factory.<locals>.charge",
                    "lineno": 41,
                    "in_app": True,
                    "vars": {"token": SENTINELS[9]},
                    "context_line": SENTINELS[5],
                    "abs_path": SENTINELS[6],
                },
            ],
            "frames_omitted": [1, 2],
        },
...
@pytest.mark.parametrize(
    ...
    [
        ("message", SENTINELS[2]),
        ("logentry", {"message": SENTINELS[2]}),
        ("culprit", "proliferate.server.billing.service in charge"),
        ("logger", "proliferate.billing"),
        ("server_name", "ip-10-0-0-1"),
        ("dist", "1"),
        ("modules", {"sentry-sdk": "2.66.1"}),
        ("debug_meta", {"images": []}),
...
@pytest.mark.parametrize(
    ...
    [
        (RELEASE, True),
        ("proliferate-server@1.2.3", True),
        ("", False),
        ("proliferate-server@1.2", False),
        ("1.2.3", False),
        (RELEASE.encode(), False),
    ],
)
...
@pytest.mark.parametrize(
    ("key", "value", "survives"),
    [
        ("surface", "cloud_api", True),
        ("surface", "desktop", False),
        ("telemetry_mode", "hosted_product", True),
        ("telemetry_mode", "self_managed", False),
        ("request_id", VALID_UUID, True),
        ("request_id", SENTINELS[12], False),
        ("http_method", "POST", True),
        ("http_method", "post", False),
        ("http_route", "/orgs/{org_id}", False),
        ("http_route", "/orgs/" + VALID_UUID, False),
        ("user_id", VALID_UUID, True),
        ("organization_id", VALID_UUID, True),
        ("cloud_workspace_id", VALID_UUID, True),
        ("cloud_target_id", VALID_UUID, True),
        ("sandbox_profile_id", VALID_UUID, True),
        ("cloud_sandbox_id", VALID_UUID, True),
        ("enrollment_key_id", VALID_UUID, True),
        ("user_id", VALID_UUID.upper(), True),
        ("support_report_id", uuid4().hex, True),
        ("support_report_id", VALID_UUID, False),
        ("tenant_id", f"user:{VALID_UUID}", True),
        ("tenant_id", f"org:{VALID_UUID}", True),
        ("tenant_id", f"team:{VALID_UUID}", False),
        ("critical_failure", "true", True),
        ("critical_failure", "True", False),
        ("domain", "billing", True),
        ("domain", "billing ", False),
        ("action", "stripe_webhook_drop", True),
        ("action", "unknown_action", False),
        ("harness_kind", "codex", True),
        ("harness_kind", "gemini", False),
        ("fn", "materialize_sandbox", True),
        ("label", f"materialize_sandbox:{VALID_UUID}", True),
        ("fn", "drop_database", False),
        ("label", f"materialize_other:{VALID_UUID}", False),
        ("worker_id", "worker-1", False),
        ("external_sandbox_id", "sbx-1", False),
        ("anyharness_workspace_id", "ws-1", False),
        ("session_id", VALID_UUID, False),
        ("interaction_id", VALID_UUID, False),
        ("command_id", VALID_UUID, False),
        ("anomaly", "slow", False),
...
@pytest.mark.parametrize(
    ("key", "value", "expected"),
    [
        ("billing_subject_id", VALID_UUID, VALID_UUID),
        ("owner_user_id", None, None),
        ("subject_id", "nope", "[redacted]"),
        ("stripe_event_id", "evt_1AbC", "evt_1AbC"),
        ("stripe_event_id", "ch_1AbC", "[redacted]"),
        ("stripe_object_id", "in_1AbC", "in_1AbC"),
        ...
        ("drop_reason", None, "[redacted]"),
        ("session_mode", "payment", "payment"),
        ("session_mode", "PAYMENT", "[redacted]"),
        ...
        ("event_type", "invoice.paid", "[redacted]"),
        ("paid", True, True),
        ("paid", 1, "[redacted]"),
        ("has_status", None, None),
        ("line_item_count", 3, 3),
        ("line_item_count", True, "[redacted]"),
        ("line_item_count", 10001, "[redacted]"),
    ],
)
...
def test_request_keeps_only_a_cataloged_method() -> None:
    request = {
        ...
        "query_string": f"{SENTINELS[8]}=1",
        "cookies": {"session": SENTINELS[9]},
        "headers": {"authorization": SENTINELS[9]},
        "env": {"SECRET": SENTINELS[10]},
    }
...
# --- malformed and defensive shapes ------------------------------------------
        ...
        "spans": [
            {
                ...
                "op": "queue.publish",
                "status": "ok",
                "start_timestamp": 1755600000.1,
                "timestamp": 1755600000.9,
                "description": SENTINELS[11],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("generic FastAPI request", "route", True),
        ("/orgs/{org_id}", "route", False),
        ("proliferate.server.billing.api.create_checkout", "route", False),
        ("generic FastAPI request", "component", False),
        ("GET /orgs/x", "url", False),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        {"message": SENTINELS[2]},
        {"data": {"prompt": SENTINELS[2]}},
        {"type": "unknown"},
        {"category": "django.request"},
        {},
        "not-a-mapping",
        None,
    ],
...
ALLOWED_PATHS = {
    ...
    ".spans[].op",
    ".spans[].status",
    ".spans[].start_timestamp",
    ".spans[].timestamp",
    ".spans[].same_process_as_parent",
    ".threads.values[].crashed",
    ".threads.values[].current",
    f".threads.values[].{_STACK}.filename",
    ".breadcrumbs.values[].type",
    ".breadcrumbs.values[].level",
    ".breadcrumbs.values[].category",
    ".breadcrumbs.values[].timestamp",
