...
from proliferate.integrations.sandbox.e2b import E2BRuntimeError, E2BUnavailableError
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            E2BRuntimeError("secret provider configuration detail"),
...
async def test_deterministic_target_failures_terminalize_without_successor(
    ...
    owner = await register_and_login(
        client,
        f"managed-permanent-{type(failure).__name__}-{expected_code}@example.com",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            E2BUnavailableError("secret provider network detail"),
            "workflow_target_unreachable",
        ),
        (
            CloudMaterializationLockUnavailable("secret redis detail"),
