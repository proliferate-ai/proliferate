...
_RETAINED_POLICY_TABLE = "cloud_worktree_retention_policy"
_RETAINED_POLICY_INDEX = "ix_cloud_worktree_retention_policy_user_id"
_UNMANAGED_SENTINEL_TABLE = "c2_unmanaged_schema_drift_sentinel"
...
@pytest.mark.asyncio
async def test_alembic_autogenerate_ignores_only_retained_policy_table() -> None:
    async with temporary_database("retained_policy_autogenerate") as (
        _database_name,
        database_url,
    ):
        await run_migrations_async(database_url)
        sentinel_engine = create_async_engine(database_url, echo=False)
        try:
            async with sentinel_engine.begin() as connection:
                await connection.execute(
                    text(f"CREATE TABLE {_UNMANAGED_SENTINEL_TABLE} (id INTEGER PRIMARY KEY)")
                )
        finally:
            await sentinel_engine.dispose()

        with pytest.raises(AutogenerateDiffsDetected) as exc_info:
            await asyncio.to_thread(command.check, build_alembic_config(database_url))

        removed_objects = {
            (diff[0], diff[1].name)
            for diff in exc_info.value.diffs
            if isinstance(diff, tuple) and diff[0] in {"remove_index", "remove_table"}
        }
        assert ("remove_table", _RETAINED_POLICY_TABLE) not in removed_objects
        assert ("remove_index", _RETAINED_POLICY_INDEX) not in removed_objects
        assert ("remove_table", _UNMANAGED_SENTINEL_TABLE) in removed_objects
