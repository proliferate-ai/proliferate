        ...
        try:
            async with inspection_engine.begin() as conn:
                ...
                organization_columns = await conn.run_sync(
                    lambda sync_conn: {
                        column["name"]
                        for column in inspect(sync_conn).get_columns("organization")
                    }
                )
                ...
                assert "uq_organization_invitation_pending_email" in (
                    organization_invitation_indexes
                )
                ...
                billing_indexes = await conn.run_sync(
                    lambda sync_conn: {
                        index["name"]
                        for index in inspect(sync_conn).get_indexes("billing_subject")
                    }
                )
