"""Integration tests for the single model-snapshot ingest route (B4 re-key).
...
cloud-sandbox Worker, and the owner is derived from its sandbox row rather than
...
def _body(models: list[str], *, probed_at: str = PROBED_AT) -> dict[str, str]:
    return {
        "authContextId": CONTEXT,
        "snapshotJson": snapshot_entry(models),
        "probedAt": probed_at,
    }
...
class TestWorkerIngest:
    ...
    @pytest.mark.asyncio
    async def test_upload_stores_the_entry_verbatim(
        self,
        client: AsyncClient,
        db_session: AsyncSession,
    ) -> None:
        """attestation/warnings/lastAttempt survive so C3 can render staleness."""
        ...
        assert stored is not None
        entry = json.loads(stored.snapshot_json)
        assert entry["attestation"] == {"name": HARNESS, "version": "1.2.3"}
        assert entry["authFingerprint"] == "sha256:9f2cdeadbeef"
        assert entry["lastAttempt"]["outcome"] == "ok"
    ...
    @pytest.mark.asyncio
    async def test_invalid_entry_payloads_are_400(
        self,
        client: AsyncClient,
        db_session: AsyncSession,
    ) -> None:
        ...
        for bad_entry in ("not-json", json.dumps(["array"]), json.dumps({"models": [{"x": 1}]})):
            response = await client.post(
                INGEST_PATH,
                json={
                    "authContextId": CONTEXT,
                    "snapshotJson": bad_entry,
                    "probedAt": PROBED_AT,
                },
                headers=worker,
            )
            assert response.status_code == 400, bad_entry
    ...
    @pytest.mark.asyncio
    async def test_invalid_scope_ids_are_400(
        self,
        client: AsyncClient,
        db_session: AsyncSession,
    ) -> None:
        ...
        long_context = await client.post(
            INGEST_PATH,
            json={
                "authContextId": "x" * 65,
                "snapshotJson": snapshot_entry(["m"]),
                "probedAt": PROBED_AT,
            },
            headers=worker,
        )
...
class TestIngestSoftVersioning:
        ...
        async def count(status: str) -> int:
            result = await db_session.execute(
                ...
                .where(
                    ...
                    AgentModelSnapshot.auth_context_id == CONTEXT,
    ...
    async def test_a_redundant_reupload_is_absorbed_idempotently(
        ...
        """A Worker restart re-sends its last entry; the write must absorb it.

        The spec accepts "at most one redundant upload after a Worker restart"
        precisely because the soft-versioned write handles it — so the same entry
        twice must leave exactly one active row, not a unique-violation 500.
        """
        user_id, _ = await authed_user(client)
        worker = await cloud_sandbox_worker_headers(
            db_session,
    ...
    async def test_two_contexts_upload_independently(
        ...
        for context, models in (("gateway", ["gw"]), ("anthropic-api", ["api"])):
            response = await client.post(
                INGEST_PATH,
                json={
                    "authContextId": context,
        ...
        assert model_ids(gateway.json()) == ["gw"]
        assert model_ids(api.json()) == ["api"]
