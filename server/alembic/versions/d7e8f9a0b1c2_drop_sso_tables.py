...
culled product surface; no data is preserved.

``downgrade`` restores the SCHEMA ONLY (the DDL is copied verbatim from
``b5c6d7e8f9a0_sso_connections``, the sole migration that ever shaped these
tables) so head-to-history downgrade proofs keep traversing this revision.
Dropped rows are unrecoverable through alembic; production deploys snapshot
the database before running migrations as a standing deploy-time step — that
