...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .controls .row { margin:2px 0; color:var(--n700); }
  .controls .k { font-size:11px; font-weight:600; color:var(--n600); margin-right:4px; }
  .val { display:inline-block; background:var(--n100); color:var(--n700);
         border-radius:6px; padding:0 7px; font-size:11.5px; margin:1px 2px; }
...
function controlCells(controls) {
  const rows = Object.entries(controls).map(([k, c]) =>
    ...
    c.values.map(v => '<span class="val' + (v === c.observedValue ? ' obs' : '') + ' mono">' + esc(v) + '</span>').join('') +
    '</div>');
  return rows.join('') || '<span style="color:var(--n500)">none reported</span>';
}
