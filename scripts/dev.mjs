...
function writeLaunchEnv(paths, env) {
  const launchEnv = {
    ...
    CORS_ALLOW_ORIGINS: [
      ...
      `http://localhost:${env.PROLIFERATE_MOBILE_WEB_PORT}`,
      `http://127.0.0.1:${env.PROLIFERATE_MOBILE_WEB_PORT}`,
...
async function ensureProfile(options) {
  ...
  const instance = {
    ...
    ports: {
      ...
      mobileWeb: Number(env.PROLIFERATE_MOBILE_WEB_PORT),
...
async function listProfiles() {
  ...
  for (const entry of readdirSync(profilesRoot, { withFileTypes: true })) {
    ...
    const checks = await Promise.all(checkedPorts.map((port) => tcpPortIsOpen(port)));
    ...
    rows.push({
      ...
      hostedWeb: ports.hostedWeb || "",
      mobileWeb: ports.mobileWeb || "",
      api: ports.api || "",
