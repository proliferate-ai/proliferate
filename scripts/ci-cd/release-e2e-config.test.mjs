...
test("staging installs the runner and caches only encrypted session state", () => {
  const block = jobBlock(release, "release-e2e-staging");
  assert.match(block, /name: Install workspace dependencies[\s\S]*?pnpm install --frozen-lockfile/);
  assert.match(block, /name: Restore encrypted staging session/);
  assert.match(block, /name: Decrypt rotated staging session/);
  assert.match(block, /name: Encrypt rotated staging session/);
  assert.match(block, /name: Save encrypted staging session/);
  assert.match(block, /secrets\.RELEASE_E2E_STAGING_SESSION_CACHE_KEY/);
