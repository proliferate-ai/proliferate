"""Synthetic-only proof of the fixed-recipient Vercel custody adapter."""
...
SPEC = importlib.util.spec_from_file_location(
    "custody", Path(__file__).with_name("seal-vercel-credential.py")
)
...
class CustodyTests(unittest.TestCase):
    ...
    def test_fixed_destination_and_large_key_id_remain_exact(self):
        ...
        self.assertEqual(result["destination"]["secret_name"], "VERCEL_TOKEN")
    ...
    def test_main_only_writes_sealed_receipt_and_does_not_inherit_token_to_git(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            env = {**environment(), "RUNNER_TEMP": directory, "VERCEL_TOKEN": secret}

            def git(*args, **kwargs):
                self.assertNotIn("VERCEL_TOKEN", os.environ)
                return subprocess.CompletedProcess(args, 0, stdout=SHA + "\n")

            with (
                patch.dict(os.environ, env, clear=True),
                patch.object(custody.subprocess, "run", git),
                patch.object(custody.sys, "argv", ["seal-vercel-credential.py"]),
                patch.object(custody, "datetime", FixedClock),
                patch.object(
                    custody, "verify_projects", return_value={"read_access_verified": True}
                ),
            ...
            receipt = Path(directory) / "vercel-custody/sealed.json"
    ...
    def test_library_errors_cannot_print_token_or_create_artifact(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            with (
                patch.dict(
                    os.environ,
                    {**environment(), "RUNNER_TEMP": directory, "VERCEL_TOKEN": secret},
                    clear=True,
                ),
                patch.object(
                    custody.subprocess,
    ...
    def test_preexisting_directory_and_extra_arguments_cannot_emit_new_receipt(self):
        with tempfile.TemporaryDirectory() as directory:
            target = Path(directory) / "vercel-custody"
            target.mkdir()
            original = target / "sealed.json"
            original.write_text("existing")
            for argv in [
                ["seal-vercel-credential.py"],
                ["seal-vercel-credential.py", "other-key"],
            ]:
                with (
                    patch.dict(
                        os.environ,
    ...
    def test_actual_project_response_requires_both_exact_ids_in_the_same_team(self):
        reads = []

        def read(secret, project):
            self.assertEqual(secret, "synthetic")
            reads.append(project)
            return {"id": project, "accountId": custody.TEAM_ID}

        proof = custody.verify_projects("synthetic", read)
        self.assertEqual(reads, list(custody.PROJECTS.values()))
        self.assertEqual(proof["project_ids"], custody.PROJECTS)
        self.assertTrue(proof["read_access_verified"])

    def test_wrong_team_or_project_and_provider_failure_refuse(self):
        for body in [
            {"id": "other", "accountId": custody.TEAM_ID},
            {"id": custody.PROJECTS["staging"], "accountId": "other"},
            {},
        ]:
            with self.assertRaises(ValueError):
                custody.verify_projects("synthetic", lambda secret, project: body)
        with self.assertRaises(RuntimeError):
            custody.verify_projects(
                "synthetic",
                lambda secret, project: (_ for _ in ()).throw(RuntimeError("unavailable")),
            )

    def test_redirect_is_not_followed(self):
        with self.assertRaises(ValueError):
            custody.NoRedirect().redirect_request(
                None, None, None, None, None, "https://other.test"
            )

    def test_provider_failure_cannot_emit_an_encrypted_receipt_or_secret(self):
        secret = "synthetic-provider-failure-token"
        stdout, stderr = io.StringIO(), io.StringIO()
        with (
            ...
            patch.object(custody.sys, "argv", ["seal-vercel-credential.py"]),
            patch.object(custody, "datetime", FixedClock),
            patch.object(custody, "verify_projects", side_effect=RuntimeError(secret)),
            redirect_stdout(stdout),
            redirect_stderr(stderr),
        ):
            self.assertEqual(custody.main(), 1)
