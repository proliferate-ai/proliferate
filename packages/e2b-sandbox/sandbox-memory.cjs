...
async function fetchEmbeddings(texts, apiKey, retried = false) {
...
async function applyTemporalDecay(results, config) {
  const now = Date.now();
  const decayed = [];
  for (const result of results) {
    ...
    let ageInDays;
    if (dateFromPath) {
      ageInDays = (now - dateFromPath.getTime()) / (1e3 * 60 * 60 * 24);
    } else if (config.memoryDir) {
      try {
        const fullPath = (0, import_node_path.join)(config.memoryDir, result.path);
        const fileStat = await (0, import_promises.stat)(fullPath);
        ageInDays = (now - fileStat.mtimeMs) / (1e3 * 60 * 60 * 24);
      } catch {
        decayed.push(result);
        continue;
      }
...
function isEvergreenMemoryPath(path) {
...
function parseMemoryDateFromPath(path) {
  const name = (0, import_node_path.basename)(path, ".md");
...
function mergeResults(vectorResults, ftsResults, vectorWeight, ftsWeight) {
...
function resolveChunkDetails(merged, store) {
  const results = [];
  for (const item of merged) {
    const row = store.db.prepare("SELECT path, start_line, end_line, text FROM chunks WHERE id = ?").get(item.id);
    if (!row) continue;
    results.push({
      path: row.path,
...
var Store = class {
  ...
  // --- File records ---
  getFileRecord(path) {
    const row = this.db.prepare("SELECT path, hash, mtime, size FROM files WHERE path = ?").get(path);
    return row;
  }
  ...
  getAllFileRecords() {
    return this.db.prepare("SELECT path, hash, mtime, size FROM files").all();
  }
  ...
  getChunksByPath(path) {
    const rows = this.db.prepare("SELECT * FROM chunks WHERE path = ?").all(path);
  ...
  // --- Embedding cache ---
  getCachedEmbedding(model, hash) {
    const row = this.db.prepare("SELECT embedding FROM embedding_cache WHERE model = ? AND hash = ?").get(model, hash);
    if (!row) return void 0;
    return JSON.parse(row.embedding);
  }
  cacheEmbedding(model, hash, embedding) {
    const now = Date.now();
    this.db.prepare(
  ...
  // --- Vector entries ---
  upsertVectorEntry(id, embedding) {
    if (!this.vectorAvailable) return;
    try {
      this.db.prepare("DELETE FROM chunks_vec WHERE id = ?").run(id);
      this.db.prepare("INSERT INTO chunks_vec (id, embedding) VALUES (?, ?)").run(id, JSON.stringify(embedding));
    } catch {
    }
  }
  deleteVectorEntriesByPath(path) {
    if (!this.vectorAvailable) return;
    try {
      const rows = this.db.prepare("SELECT id FROM chunks WHERE path = ?").all(path);
...
function buildFtsQuery(raw) {
  const tokens = raw.toLowerCase().replace(/[^\w\s]/g, " ").split(/\s+/).filter((t) => t.length > 0);
...
var TARGET_TOKENS = 400;
var OVERLAP_TOKENS = 80;
...
function splitSection(lines, startLine) {
...
async function syncFiles(params) {
  ...
  for (const file of mdFiles) {
    ...
    const fileStat = await (0, import_promises2.stat)(file.absolutePath);
    const existing = existingPaths.has(file.relativePath) ? store.getFileRecord(file.relativePath) : void 0;
    ...
    const textsToEmbed = [];
    const chunkEmbeddings = new Array(rawChunks.length);
...
async function walkDir(baseDir, currentDir, results) {
  ...
  for (const entry of entries) {
    const fullPath = (0, import_node_path2.join)(currentDir, entry.name);
    if (entry.isDirectory()) {
      ...
      results.push({
        ...
        relativePath: (0, import_node_path2.relative)(baseDir, fullPath)
      });
    }
  }
}
...
var ReaddirpStream = class extends import_node_stream.Readable {
  ...
  async _exploreDir(path, depth) {
    let files;
    try {
      files = await (0, import_promises3.readdir)(path, this._rdOptions);
    } catch (error) {
      this._onError(error);
    }
  ...
  async _formatEntry(dirent, path) {
    ...
    try {
      const fullPath = (0, import_node_path3.resolve)((0, import_node_path3.join)(path, basename4));
  ...
  if (cont) {
    ...
    watcher.on(EV.ERROR, async (error) => {
      ...
      if (isWindows && error.code === "EPERM") {
        try {
          const fd = await (0, import_promises4.open)(path, "r");
          await fd.close();
          broadcastErr(error);
        } catch (err) {
        }
...
var NodeFsHandler = class {
  ...
  async _handleSymlink(entry, directory, path, item) {
    ...
    if (!this.fsw.options.followSymlinks) {
      ...
      let linkPath;
      try {
        linkPath = await (0, import_promises4.realpath)(path);
      } catch (e) {
        this.fsw._emitReady();
        return true;
      }
  ...
  async _addToNodeFs(path, initialAdd, priorWh, depth, target) {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        const targetPath = follow ? await (0, import_promises4.realpath)(path) : path;
        if (this.fsw.closed)
          return;
...
var MemoryManager = class {
  ...
  async search(query, maxResults) {
    ...
    const embedQueryFn = this.openaiApiKey ? (text) => embedQuery(text, this.openaiApiKey) : null;
    return hybridSearch({
      query,
      store: this.store,
      embedQueryFn,
  ...
  async get(path, from, lines) {
    const fullPath = (0, import_node_path4.join)(this.memoryDir, path);
    const content = await (0, import_promises6.readFile)(fullPath, "utf-8");
    if (from !== void 0 || lines !== void 0) {
  ...
  async sync() {
    ...
    const embedFn = apiKey ? (texts) => embedBatch(texts, apiKey) : async (_texts) => _texts.map(() => []);
    await syncFiles({
      memoryDir: this.memoryDir,
      store: this.store,
      embedFn
