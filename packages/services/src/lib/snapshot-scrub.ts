...
function toError(message: string, err: unknown): Error {
	const withCause = (error: Error): Error => {
		(error as Error & { cause?: unknown }).cause = err;
		return error;
	};
	if (err instanceof Error) {
		return withCause(new Error(`${message}: ${err.message}`));
	}
	return withCause(new Error(`${message}: ${String(err)}`));
}
...
export async function prepareForSnapshot(
	...
	try {
		...
		if (scrubResult.exitCode !== 0) {
			throw new Error(`env scrub failed: exit code ${scrubResult.exitCode}`);
		}
