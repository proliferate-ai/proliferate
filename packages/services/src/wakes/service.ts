...
export async function transitionWakeStatus(input: {
	...
	const wake = await wakesDb.findWakeEventByIdForOrg(input.wakeEventId, input.organizationId);
	if (!wake) {
...
export async function isTerminalWake(wakeEventId: string): Promise<boolean> {
	const wake = await wakesDb.findWakeEventById(wakeEventId);
