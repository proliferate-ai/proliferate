...
export interface WorkflowRunArgsModalProps {
  ...
  error?: string | null;
  onClose: () => void;
  onSubmit: (input: { args: Record<string, ArgValue>; targetMode: TargetMode }) => void;
}
...
/** Args form + run-location selector shown when a workflow declares args (spec 3.6). */
export function WorkflowRunArgsModal({
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ args: resolved, targetMode });
  };
    ...
    <ModalShell
      ...
      footer={
        <>
          ...
          <Button onClick={handleSubmit} loading={busy} disabled={missingRequired}>
            Run
          </Button>
        </>
      }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5 border-t border-border/60 pt-3">
          ...
          <Select value={targetMode} onChange={(event) => setTargetMode(event.target.value as TargetMode)}>
            ...
            <option value="personal_cloud">Cloud</option>
          </Select>
        </div>
      </div>
    </ModalShell>
