...
describe("cloud MCP connector persistence", () => {
  ...
  it("does not publicize org-owned configured items", async () => {
    mocks.getCloudMcpCatalogMock.mockResolvedValue({
      catalogVersion: "test",
      entries: [secretCatalogEntry("github")],
      pluginPackages: [githubPluginPackage()],
    ...
    expect(mocks.publicizeCloudMcpConnectionMock).not.toHaveBeenCalled();
    expect(mocks.unpublicizeCloudMcpConnectionMock).not.toHaveBeenCalled();
    expect(mocks.patchConfiguredPluginMock).not.toHaveBeenCalled();
    expect(mocks.patchConfiguredSkillMock).not.toHaveBeenCalled();
  });
