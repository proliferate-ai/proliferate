...
This plan is based on the current code, not the prior writeup. Pre-change baseline checks are green: \`cd apps/desktop && pnpm exec tsc --noEmit\` and \`cargo check --manifest-path src-tauri/Cargo.toml\`.

## Implementation Changes

### 1. Replace ad hoc shortcut registration
**Files:** \`apps/desktop/src/hooks/shortcuts/use-global-shortcuts.ts\`, \`apps/desktop/src/config/shortcuts.ts\`
...
**File:** new \`apps/desktop/src/lib/keyboard/dispatcher.ts\`
...
Wire \`CmdOrCtrl+,\` to navigate to \`/settings\`. The accelerator surface should show in the macOS menu as well — update \`apps/desktop/src-tauri/src/menu.rs\` to register a Settings menu item with the same accelerator.
