...
export function useLayoutGate(options: LayoutGateOptions = {}) {
	...
	const needsOnboarding = requireOnboarding && onboardingStatus
		? !onboardingStatus.onboardingComplete
		: false;

	const needsBillingSetup = requireBilling && billingEnabled
		&& billingInfo?.state.billingState === "unconfigured";
