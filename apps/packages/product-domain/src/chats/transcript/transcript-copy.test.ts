...
describe("buildTranscriptCopyText", () => {
  ...
  it("serializes plan references, non-transient thoughts, and tool command details", () => {
    ...
    const command = parsedCommandItem("command", "turn-1", 3, [{
      type: "search",
      cmd: "rg buildTranscriptCopyText apps/desktop/src",
    ...
    expect(copied).toContain("rg buildTranscriptCopyText apps/desktop/src");
    expect(copied).toContain("apps/desktop/src");
  });
