...
export function ProductSidebarNavRow({
  ...
  return (
    <SidebarRowSurface
      ...
      className={`min-h-[calc(1lh+0.5rem)] gap-2 px-2 py-1 text-[13px] leading-5 focus-visible:outline-offset-[-2px] ${className}`}
      {...props}
    >
      <div className="flex size-[17px] shrink-0 items-center justify-center [&>svg]:size-full [&>svg]:shrink-0">
        {item.icon}
      </div>
      <div className="flex min-w-0 flex-1 items-center text-[13px] leading-5 text-current">
