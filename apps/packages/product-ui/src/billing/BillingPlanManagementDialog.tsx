...
export function BillingPlanManagementDialog({
  ...
  return (
    <ModalShell
      ...
      onClose={onClose}
      title="Manage plan"
      description="Review the current Core checkout, billing portal access, and Enterprise options."
        ...
        <section className="space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <h3 className="text-sm font-medium text-foreground">Core organization plan</h3>
              <p className="max-w-xl text-xs leading-5 text-muted-foreground">
                {organizationName
                  ? `Applies to cloud work, shared workflows, and billing for ${organizationName}.`
                  : "Create an organization before upgrading to Core."}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0 space-y-1">
                <div className="text-sm font-medium text-foreground">
                  {isCore ? "Manage Core in Stripe" : "Upgrade through Stripe"}
                </div>
                <p className="text-xs leading-5 text-muted-foreground">
                  {isCore
                    ? "Open the billing portal to review invoices, payment method, and plan details."
                    : "Start checkout for the currently available Core organization plan."}
                </p>
              </div>
              <div className="text-xs text-muted-foreground">
                {isCore ? "Active plan" : "Organization checkout"}
              </div>
            </div>
          </div>
