...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaPropsWithVariant>(
  function Textarea({ className = "", variant = "default", ...props }: TextareaPropsWithVariant, ref) {
    const base = variant === "ghost"
      ...
      : variant === "flush"
        ? "w-full resize-none rounded-none border-0 bg-transparent px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground transition-colors focus:outline-none focus:ring-1 focus:ring-inset focus:ring-ring disabled:opacity-60"
        : variant === "code"
