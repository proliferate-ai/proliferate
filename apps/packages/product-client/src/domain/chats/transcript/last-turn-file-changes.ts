...
export function collectTurnTouchedFiles(
  ...
  const recordedByPath = new Map(
    collectTurnFilePatches(turn, transcript).map((file) => [file.path, {
      additions: file.additions,
      deletions: file.deletions,
      patch: file.patches.filter((patch) => patch.trim().length > 0).join("\n") || null,
    }]),
  );
...
function touchedFileFromPart(
  ...
  LastTurnTouchedFile,
  "recordedAdditions" | "recordedDeletions" | "recordedPatch"
> | null {
