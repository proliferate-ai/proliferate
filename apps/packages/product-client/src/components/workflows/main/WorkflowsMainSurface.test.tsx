...
describe("WorkflowsMainSurface", () => {
  ...
  it("opens the attached blank/template launcher from the create row", () => {
    ...
    expect(screen.queryByRole("dialog", { name: "Create a new workflow" })).toBeNull();
    fireEvent.click(screen.getByRole("button", { name: /Create a new workflow/ }));
    expect(screen.getByRole("dialog", { name: "Create a new workflow" })).toBeTruthy();
...
describe("WorkflowsMainSurface legacy group", () => {
  function withLegacy() {
    mocks.listQuery.data = {
      workflows: [
        ...
        listRow({
          ...
          description: "Saved on the old editor.",
          revision: 9,
          schemaVersion: 1,
        }),
      ],
  ...
  it("surfaces a gen-1 definition under a Legacy heading with a v1 badge", () => {
    withLegacy();
  ...
  it("gives a legacy row delete as its only affordance", () => {
    withLegacy();
    ...
    // A v1 definition cannot open in the v2 builder or be placed as a v2
    // invocation, so the row must offer neither.
    expect(screen.queryByRole("button", { name: "Run Legacy workflow" })).toBeNull();
    expect(screen.queryByRole("button", { name: "Edit Legacy workflow" })).toBeNull();
    expect(screen.queryByRole("button", { name: "Legacy workflow actions" })).toBeNull();
  ...
  it("deletes a legacy definition through the shared definitions mutation", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Delete Legacy workflow" }));
    expect(screen.getByText("Delete this workflow?")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));

    await waitFor(() => expect(mocks.deleteWorkflowDefinitionV2).toHaveBeenCalledWith({
      workflowDefinitionId: "wf-legacy",
      expectedRevision: 9,
    }));
  });

  it("renders no legacy group when every row is gen-2", () => {
    mocks.listQuery.data = { workflows: [listRow()] };
