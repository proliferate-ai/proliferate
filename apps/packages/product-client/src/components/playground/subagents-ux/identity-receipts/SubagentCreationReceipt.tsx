    ...
    >
      ...
      <div className={`flex min-w-0 max-w-full items-center ${compact ? "gap-1.5" : "gap-2"}`}>
        <Button
          ...
          className={`group/receipt inline-flex min-w-0 max-w-64 items-center rounded-full border border-border/60 bg-foreground/5 hover:border-border hover:bg-hover active:bg-active focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-border ${
            compact ? "h-6 gap-1 pl-1.5 pr-2" : "h-7 gap-1.5 pl-2 pr-2.5"
          }`}
        >
          <SubagentIdentityGlyph
            seed={model.subagentId}
            className={compact ? "text-ui-sm icon-compact" : "text-chat icon-paired"}
            label={`Identity mark for ${model.title}`}
          />
          <span
            className={`truncate font-medium text-foreground ${compact ? "text-ui-sm" : "text-ui"}`}
            title={model.title}
          >
            {model.title}
          </span>
          {detailsOpen
            ? <ChevronDown className="text-ui icon-compact shrink-0 text-muted-foreground" aria-hidden="true" />
            : (
              <ChevronRight
                className="text-ui icon-compact shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover/receipt:opacity-100 group-focus-visible/receipt:opacity-100"
                aria-hidden="true"
              />
            )}
        </Button>
      ...
      {detailsOpen && (
        <div
          id={detailsId}
          className={`mt-1 flex w-fit max-w-full flex-col gap-1 rounded-md border border-border/60 bg-foreground/5 text-ui-sm text-muted-foreground ${
