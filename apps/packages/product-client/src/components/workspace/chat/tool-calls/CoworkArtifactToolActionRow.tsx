...
function ArtifactChip({
  ...
  if (!onClick) {
    return (
      <span
        title={pathLabel}
        className="inline-flex min-w-0 max-w-full items-center gap-0.5 rounded-sm border border-border/60 bg-muted/45 px-1 py-px text-chat leading-none text-foreground/90"
      >
        {chipContent}
      </span>
    );
  ...
  return (
    <Button
      ...
      className="h-auto max-w-full rounded-sm border border-border/60 bg-muted/45 px-1 py-px text-chat leading-none text-foreground/90 hover:bg-muted"
    >
      {chipContent}
    </Button>
  );
