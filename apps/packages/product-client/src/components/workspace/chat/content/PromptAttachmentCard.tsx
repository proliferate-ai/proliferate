    ...
    <div className={className} data-telemetry-mask title={title}>
      {canPreview && onOpenAttachment ? (
        <Button
          ...
          className="absolute inset-0 z-0 h-full w-full rounded-inherit bg-transparent p-0 hover:bg-hover active:bg-active focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-inset"
  ...
  return (
    ...
    >
      {image.isLoading ? (
        ...
        <FileIcon className="icon-paired text-muted-foreground [font-size:var(--text-chat)]" />
      )}
    </div>
  );
}
