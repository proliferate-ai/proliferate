...
export function projectRecentSummaries(
  window: Extract<DecodedSupportWindow, { state: "decoded" }>,
  selection: Extract<BundledLocalSupportSelection, { kind: "recent_activity" }>,
  ...
  let previousUpdatedAt: ReturnType<typeof timestampInstant> | null = null;
  let previousId = "";
...
export function projectActiveSummary(
  result: SettledMeasured,
  selection: Extract<BundledLocalSupportSelection, { kind: "active_session" }>,
  preparation: SupportSnapshotPreparation,
  budget: SupportProjectionBudget,
...
export function sessionListEndpoint(
...
export function sessionListFromSummary(
...
export function projectListEndpoint(
  result: SettledMeasured,
