 ...
 *
 * `nodeIndexTitle` and `kindLine` are copy *factories*, not presentation
...
export const WORKFLOW_NODE_CARD_COPY = {
  /**
   * "03 · Research the topic" — one-based, zero-padded to two digits. A row
   * with no chain position (contractually impossible for a defined node, but
   * a bad projection should stay visible rather than throw) renders "--"
   * instead of a bogus number.
   */
  nodeIndexTitle: (chainIndex: number | null, title: string): string => {
    const indexLabel = chainIndex === null
      ? "--"
      : String(chainIndex + 1).padStart(2, "0");
    return `${indexLabel} · ${title}`;
  },
  /** "Agent" / "Human gate", annotated with why this row exists at all. */
  kindLine: (
    nodeType: "agent" | "human_in_loop",
    kind: "defined" | "replacement" | "adhoc",
  ): string => {
    const base = nodeType === "agent" ? "Agent" : "Human gate";
