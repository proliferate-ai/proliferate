...
export function CloudAgentAuthLibrary({ initialAgentKind = null }: CloudAgentAuthLibraryProps) {
  ...
  const credentialFormRef = useRef<HTMLDivElement | null>(null);
  const initialAgentKindAppliedRef = useRef(false);
  ...
  const [credentialFormFocusRequest, setCredentialFormFocusRequest] = useState<{
  ...
  useEffect(() => {
    if (initialAgentKind) {
      if (initialAgentKindAppliedRef.current) {
        return;
      }
      initialAgentKindAppliedRef.current = true;
      setCredentialFormFocusRequest({
        agentKind: initialAgentKind,
        requestId: Date.now(),
      });
      credentialFormRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [initialAgentKind]);
  ...
  function handleAddCredential(agentKind: AgentAuthAgentKind) {
    setCredentialFormFocusRequest({
      agentKind,
      requestId: Date.now(),
    });
    ...
    <div className="space-y-6">
      ...
      <div ref={credentialFormRef}>
        <CloudAgentAuthCredentialForm
          ...
          focusRequest={credentialFormFocusRequest}
        />
      </div>
