...
- **The code split** (move-only, zero behavior): the gateway files out of `server/proliferate/server/agent_auth/` — enrollment, free_credits, budget, usage_import, topups, verification, migration, signup_hook, worker, and api.py's `/agent-gateway` routes — into `server/proliferate/server/ai_gateway/` with its own `MANIFEST.toml`. The agent_auth remainder recomposes per agent_auth §4 cell 1's tree (api.py · models.py · selection_rules.py · state_render.py · service.py · harness_settings.py — vault CRUD stays in `db/store/agent_gateway/api_keys.py`, and `seats.py` arrives with slice 4, not here). Import sites updated mechanically; `git log --follow` must survive (moves, not delete+create).
