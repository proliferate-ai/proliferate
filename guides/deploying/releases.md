...
- there is no bundled agent seed on Windows. `agent-seed.inputs.json` pins Node only for the two Apple targets and `scripts/build-agent-seed.mjs` is native-macOS-only, so a Windows build sets `ANYHARNESS_AGENT_SEED_EXPECTED` with no seed present. The managed agent launcher is Windows-capable since PR #2152 (a `.cmd` launcher replaces the POSIX `#!/bin/sh` script on Windows), and agent install, spawn, launcher execution, and ACP initialize were proven on a real Windows build in PR #2165, so agent sessions do start on Windows builds when a Node runtime is already present on the machine.
