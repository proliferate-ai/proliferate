...
2. **AMG has no usable native Grafana "email" contact-point type.**
   ...
   the protected environment value) and prepends a child route that mirrors
   the unique live SNS delivery child's match shape with `continue: true`.
   Matching the two sibling routes makes the delivery a real fan-out even
   when AMG wraps its managed routes in an autogenerated matcher. The root
