...
| --- | --- | --- | --- | --- |
| Clippy | anyharness | root `Cargo.toml` `[workspace.lints.clippy]` (every member crate: `[lints] workspace = true`) | ci.yml `rust-lint` — `cargo clippy --workspace --all-targets -- -D warnings` (since 2026-08-27) | Rust idiom + correctness lints, warnings are errors. The allow-list is the constitution: four judgment lints (`result_large_err`, `too_many_arguments`, `type_complexity`, `large_enum_variant`) allowed **provisionally**, re-ruled at the code-debt migration; `await_holding_lock` allowed with a tracked debt record — `lints/anyharness/native-debt.toml` AH-CLIPPY-1, #2276 |
| rustfmt | anyharness | workspace defaults | **pending the format train-end**: the tree is not fmt-clean (223 files drift as of 2026-08-27 — the earlier "clean" reading was a false negative from a machine without rustfmt installed); the repo-wide `cargo fmt --all` apply lands with the Biome format commit, and `cargo fmt --all --check` joins ci.yml `rust-lint` in that same PR. Until then: `make fmt` locally | Rust formatting |
