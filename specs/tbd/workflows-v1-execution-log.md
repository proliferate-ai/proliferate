...
| 13* | WS3b credential audiences (VERIFIED, stacked — merge paused) | `1da38eab4` → rebased `4b64e2692` | WS2b; onto `8b5d6dcbd` | agent: 74 tests; captain post-rebase: 54 tests + head e5f1a2b3c4d7 (after Docker/DB restart) | VERIFIED-PENDING-MERGE — four audiences + issuance/exchange/rotate; legacy NULL-audience compat; api.py allowlist lowered 471→434 |
| 14* | WS4a Beat schedules + outbox (VERIFIED, stacked — merge paused) | `e8cc7fe33` → rebased `9687cc59f` onto WS3b tip | WS2b/WS2a; stacked on `4b64e2692` | agent: 79 tests + boot-smoke; captain: 41 tests indep + 22 post-stack + head e5f1a2b3c4d7 | VERIFIED-PENDING-MERGE — beat/task/commit-free service, DST fixed to spec, WORKFLOWS_BEAT_SCHEDULES flag (default off), triggers.py debt 1530→284 + entry removed. FINDING: legacy automations worker entrypoint cannot boot in prod (CloudRepoConfig deleted b21db66c4) → scheduled workflow firing likely dead in prod; WS4c beat-cutover is the fix. WS4c also owns: relay dead-letter cap, actions debt split |
| 17* | WS2c delivery/reconcile (VERIFIED, stacked — merge paused) | `39dba1d63` → rebased `979724a70` onto WS4a tip | WS3b+WS2b+WS2a; stacked on `9687cc59f` | agent: 71 tests ×2 + heads e5f1a2b3c4d7 + max-lines + boundaries + ruff; captain: full diff review + post-rebase 71/71 (clean-DB rerun after a shared-DB race false alarm) + heads | VERIFIED-PENDING-MERGE — commit-before-delivery (transactions.commit_then_deliver_cloud_run, §10.2), revisioned POST /runs/{id}/observations with full §5.4 CAS matrix (applied/retry_noop/stale/future 409+ackedRevision/conflict audited/terminal immutable), state-axis write map (legacy status stays authoritative), §8.3 two-branch cancel (pre-acceptance: no fabricated observation + outbox invalidation + reserved-lease release; post-claim: durable control command w/ delivery identity), T1-WF-LEDGER-01. CLEARS Gate A1 blocker: test_workflow_service.py 847→205 split + allowlist entry removed. Allowlist bumps documented (+42 store, +46 api, +15 models). WS5c replay contract: 409 → resend from ackedRevision+1; legacy sentinels ''/''/0 only when columns NULL |
| 15 | WS1-followup float canonicalization | `8d5474186` (merged `eaa4f896b`) | WS1/WS2b blocker ruling | agent: checker 3-leg pass, 20 server tests, 666 PD tests, 200k-double fuzz vs Node; captain: diff review + vector spot-check + checker re-run | ACCEPTED — RFC 8785 §3.2.2.3 ECMA placement rules in Python (repr digits + ES thresholds), TS native String() conformance, 27 shared vectors in the checker. CLEARS the fractional-float StartRun blocker |
| 16 | WS9a-addendum PD follow-ups | `977e1fefc` (rebased `509cb7c4f`, merged) | WS9a/WS9b | agent: 675 PD tests, desktop tsc, drag suites; captain: diff review + post-rebase 676 tests | ACCEPTED — emit-step requiredInvocation + validation parity, wire<->CapabilityRef converters, shared ensureDefinitionIds (desktop delegates). MODEL NOTE: implementation on sonnet, captain diff-review acceptance (new operating model) |
...
Note: a first post-rebase WS2c run showed 71 ERRORs — root-caused to two pytest
sessions (captain rerun + agent's own final verify) racing the shared dev
Postgres, not code; both sampled tests pass in isolation and the clean rerun is
71/71. Lesson recorded: never run two workflow-suite sessions against one DB.
...
- Gate A1: test_workflow_service.py split assigned to WS2c (in flight).
- RESOLVED: float canonicalization fixed (packet 15).
