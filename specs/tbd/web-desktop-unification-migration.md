...
Status: decision-complete migration plan. This document describes the intended
cutover, not the current operating law. The implementation PR must promote the
settled contract into `specs/codebase/**` and update the existing frontend
structure docs at the same time that the code starts following it.
...
- its public export map points at the small set of source entrypoints the hosts
  need (`ProductClient`, provider, and host types);
...
handlers and redirects. After R smoke passes, audit/update dashboard and
deployed-environment registrations whose canonical destination changed;
Desktop-aware billing returns continue to use the retained decoder. Keep the
...
and measure `/login` and authenticated `/`. The shared candidate root—not the
throwaway old Web root—defines:
...
2. Promote the package contract into canonical frontend docs.
...
9. Build the in-place ProductClient split/fixture from Section 13.1, define the
   readiness markers there, and prove or explicitly revise the byte targets
   before authorizing the mechanical move.
10. Prove Web-mode composition can exist with `desktop: null` and without
   local runtime discovery, but do not migrate or embellish the old Web
   product here.
...
Before the freeze, merge or explicitly park every live Desktop UI branch;
announce the cutover commit and do not accept new hot-surface changes until
the gate is green.
...
PR 3 must pass Web build/typecheck/CI. It is a review-only stacked diff based
on PR 2 and is never merged to `main` independently.
...
after both diffs are approved, one combined landing PR/commit containing PR 3
and PR 4 is merged to `main`. CI rejects a production Web artifact that lacks
the ProductClient mount. This preserves separate review units without ever
putting the holding host on `main` or staging/production.
...
- immediately before PR 2: inventory every open PR/worktree touching Desktop,
  ...
  explicitly park or retarget the rest;
...
| PR 1 | product-client import-map proof; Desktop and Web typecheck/build/tests; shared packages; structure and boundary scripts prove every relevant rule scanned product-client; in-place ProductClient split/readiness/performance feasibility build |
...
- [`core-release-validation.md`](../developing/testing/core-release-validation.md)
  for required guarantees, cell semantics, and fail-closed gates;
- [`release-worlds-and-fixtures.md`](../developing/testing/release-worlds-and-fixtures.md)
  for artifacts, topology, readiness, and fixture lifetime;
- [`tier-3-scenario-contract.md`](../developing/testing/tier-3-scenario-contract.md)
  for exact local-runtime, managed-cloud, billing, and self-host journeys; and
- [`tier-4-scenario-contract.md`](../developing/testing/tier-4-scenario-contract.md)
  for the two standing N-1→N journeys.
...
- PR 2 may be reverted as one mechanical unit while the old Web product still
  ...
  cutover gate.
- PR 3 is never merged to `main` independently. PR 3 and PR 4 are reviewed as
  separate stacked diffs, then combined into one mainline landing commit/PR;
  CI refuses any production Web build without the ProductClient mount.
- Deploy Vercel redirects before or with PR 4, then update external/deployed
  URL producers using the release ledger. Retain the redirects through R+1
...
The promoted feature contract should live at:

```text
specs/codebase/features/web-desktop-client-unification.md
```

This TBD plan can then be removed or reduced to any unfinished rollout ledger.
