...
Status: TBD routing record. The numbered spec pack has been split across
`specs/codebase/primitives/` and `specs/codebase/features/`; keep this file only until its glossary
and dependency-order notes have been folded into the owning docs.

Date: 2026-05-20.

Use `specs/codebase/primitives/sandbox-provisioning.md` as the foundation. Every other
cloud, sandbox, auth, command, or dispatch doc assumes that primitive.

## Files

```text
specs/codebase/primitives/sandbox-provisioning.md   Cloud target / managed sandbox foundation.
specs/codebase/primitives/mcp-skills.md             Sandbox-scoped MCP/skill/plugin runtime config.
specs/codebase/primitives/agent-auth.md             Sandbox-scoped agent LLM auth and gateway.
specs/codebase/features/settings-admin-ia.md        Settings/Admin information architecture and shared UI primitives.
specs/codebase/primitives/cloud-commands.md         Cloud command queue, worker dispatch, projection, preflight.
specs/codebase/primitives/claiming.md               Shared unclaimed work, claim ownership, Desktop direct-attach tokens.
specs/codebase/features/automations.md              Scheduled/triggered work that reuses sandbox/auth/command primitives.
specs/codebase/features/slack-bot.md                Slack bot as a team-automation entrypoint.
specs/codebase/features/cloud-dispatch.md           Cloud-mediated web/mobile clients and remote-access UX.
specs/codebase/primitives/billing.md                Entitlement, compute usage, managed credits, runtime blocks.
specs/codebase/features/workspace-migration.md      Move runnable workspace/session state between targets.
```
