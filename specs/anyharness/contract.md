...
| --- | --- | --- |
| read active catalog version | `GET /v1/catalogs/agents/version` | Read-only observability; the only catalogs route the runtime exposes. There is no apply/push route — the catalog is binary-only ([agent-distribution.md](../codebase/platforms/product/agent-distribution.md) "Convergence"). |
