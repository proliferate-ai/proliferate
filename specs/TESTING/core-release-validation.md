...
| `T2-DELEGATE-1` | Subagent items share canonical identity/status ordering across tabs, popovers, sidebar, transcript, and details. Parent provenance, wake receipts, raw-id hiding, contiguous hierarchy, close/delete confirmation, and parent-composer availability remain correct. |
...
| `T3-DELEGATE-UI-1` | In the Desktop product renderer, live subagent work agrees across tabs, Agents popover, sidebar, transcript receipts, and details. Canonical identity/status, contiguous hierarchy, breadcrumb, attention filtering, close-tab versus end-work confirmation, page reload, and transcript retention hold. |
...
| `T4-DELEGATE-1` | Session-link, subagent, wake, or prompt-policy changes | Upgrade while children run, messages queue, and wakes arm. Handles, provenance, cursors, prompts, results, hierarchy, and closure survive; each queued effect occurs once and old tokens cannot widen scope. |
