...
| `T3-SUBAGENT-1` | Workspace MCP exposes exactly 18 tools and proves one valid call for each against eligible Standard ordinary, delegated, and promoted/restarted sessions; `InternalOnly` and Cowork sessions receive no Workspace attachment. Request-time authority denies delegated creation and invalid parent, relationship, or cross-workspace targets; per-turn product context changes on promotion; create, send, Close, Open, Promote, and durable completion relay survive busy queues, restart, lost response, and replay exactly once. No legacy Subagents MCP name or delegated-agent wake schedule remains, while Cowork wake mobility still passes. |
