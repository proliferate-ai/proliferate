...
| `T2-MODELREG-1` | Bundled catalog, trusted registry snapshot, cloud projection, and seeded ACP projection remain distinct truths. Alias canonicalization, opt-in/visibility overrides, last-visible protection, stale saved intent, explicit fallback, target/workspace scope, refresh errors, and shared chat/Automation defaults never silently substitute a model. |
...
| ID | Required validation |
| --- | --- |
| `T3-CHAT-1` | In the Desktop-local runtime world, every probed, registry-supported, target-compatible harness installs its trusted pin, selects its qualification-policy model (cheapest eligible by default; exact `gpt-5.2` for Codex), completes a bounded turn, and reopens history. Managed cloud and self-host use one representative compatible harness to prove their distinct wiring rather than repeating the harness Cartesian product. Cataloged-but-incompatible entries return truthful typed unsupported/readiness state; every resolved supported matrix cell is independently required. Under the temporary 2026-07-22 ruling, Grok chat-spend is unsupported and cannot satisfy this row. |
...
| `T3-CFG-1` | Create-time model/mode/defaults derive from the bundled catalog. In an existing session, mutate only controls and values advertised by live ACP, read back exactly, apply on the next turn, and survive reconnect; stale/unknown values fail without mutating state. Under the temporary 2026-07-22 ruling, Grok is unsupported because its live qualification probe exposes one model and no independent settable control. |
...
| `T4-MODELREG-1` | Model catalog, registry, alias, visibility, or saved-intent changes | N-1 saved intents, aliases, snapshots, visibility overrides, and Automation defaults resolve under N without silent model-class substitution. Renames canonicalize; removed/unavailable models surface repair or only the explicitly stored fallback. |
