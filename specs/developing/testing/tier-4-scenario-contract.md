...
Status: agreed design contract for the core Tier 4 upgrade guarantee. This
...
user-owned artifact permutations. Focused lower-tier tests own those ownership,
path-safety, checksum, retry, and rollback decision matrices. When a live
updater mechanism itself changes, its triggered Tier 4 row may add the smallest
real failure injection needed to prove activation remains fail-closed.
...
candidate N qualification API
  ...
  + disposable actor, enrollment, and target
```
