...
specified and tested; that does not weaken Supervisor's ownership of Worker and
AnyHarness component updates.
...
and production-promotion consumption remain absent, so these workflows do not
satisfy [`core-release-validation.md`](core-release-validation.md). The
harnesses live in `tests/intent/` and `tests/release/`.
