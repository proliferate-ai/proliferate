...
Status: authoritative for automated Tier 2, Tier 3, and Tier 4 product
qualification.

This document is the complete target manifest for deciding whether a
Proliferate release works as intended. It owns the required journeys and the
...
- [`README.md`](README.md) owns tier mechanics, placement, and test-writing
  rules.
- [`flows.md`](flows.md) maps the currently implemented subset to collected
...
- [`core-release-scenario-manifest.json`](core-release-scenario-manifest.json)
  is the machine-checked target scenario-id inventory. It does not yet map all
  ids to collected tests and lanes.
...
2. Every Tier 2 row in this document passes on the merge candidate or exact
   merged SHA.
3. Every Tier 3 row in this document passes against the deployed candidate.
...
| Gate | Required work | Result policy |
| --- | --- | --- |
...
| Staging qualification | Deploy immutable artifacts for the exact merged SHA; run the complete Tier 3 manifest and triggered Tier 4 rows | The aggregate contains exactly one final green result per required ID/lane. All attempts remain visible; duplicate final claims or effects fail qualification. |
...
models. The machine manifest assigns every row an explicit lane: local runtime,
packaged native Desktop, deployed cloud/E2B, hosted Web, mobile Web, iOS,
Android, self-hosted, or a declared combination. No row silently inherits a
local or cloud lane, and matrices enumerate only routes valid for that target.
...
| `T3-INT-1` | Every harness in both lanes calls a real API-key integration through the gateway; audit records exact provider/tool/outcome; disable/revoke produces zero upstream calls and an enumerated failure. |
...
| --- | --- | --- |
| `T4-WORKER-1` | Worker artifact or update protocol changes | N-1 Worker observes desired N, writes the atomic mailbox request, and stays out of download/swap/restart ownership. The specified black-box target update owner verifies/activates N; N reports convergence and preserves identity, cursors, pending results, and a live session. Corrupt/unhealthy N leaves or restores the last-good Worker. |
| `T4-RUNTIME-1` | AnyHarness artifact or runtime-update protocol changes | Worker observes desired runtime N and requests rather than performs activation. The specified target update owner quiesces, verifies, activates, health-gates, and if needed rolls back; Worker reconnects/reconciles, the durable session resumes, and event sequence remains monotonic. |
| `T4-SUPERVISOR-1` | Supervisor artifact, config, install layout, service, or update-staging changes | N Supervisor verifies and privately stages handed component artifacts, rejects invalid component/path/size/checksum, and boots managed-cloud and SSH-generated service/config layouts with explicit child environment and correct dependency order. No self-upgrade activation is claimed until its owner is specified and tested. |
...
scenario ids; CI enforces that equality, uniqueness, tier counts, and local
link integrity. Full qualification additionally requires an execution
manifest that validates that:
...
- every test pointer in [`flows.md`](flows.md) exists;
- every implemented Tier 2/3/4 core scenario maps back to one id here.
...
The current workflows are a migration exception. The public-dependency intent
job executes on every PR. The Stripe-bearing billing job executes for trusted
same-repository PRs and for `merge_group`, `main`, and manual events; fork and
Dependabot PRs receive an explicit non-qualifying job skip because GitHub does
not expose the repository secret to their code. The protected-branch contract
therefore depends on the trusted merge-queue rerun, where both collected jobs
execute.

Both executed Tier 2 jobs fail closed over the tests they collect: missing or
invalid Stripe test credentials, a required runtime failure, an assertion
failure, a skipped/expected-fail final result, or a flaky retry leaves the job
red. They still do not collect or enforce all 68 Tier 2 rows in this document.
The machine target inventory prevents scenario-id drift, but no complete
execution/lane map proves collection completeness. Repository rulesets must
also require the trusted integration checks; workflow YAML cannot do that.

Release-E2E jobs preserve genuine runner failures. Manual dispatch defaults to
release policy, which rejects every non-green or duplicate emitted registered
result and any row missing from its 12-row provisional Workflow manifest. The
scheduled local/source-built run uses signal policy, staging is dispatch-only,
and the self-host artifact job is only a post-publish diagnostic. None is a
WS10c qualification or promotion artifact. The complete Tier 3/Tier 4
execution manifest, signed product-wide aggregator, required-manifest hash,
exact artifact identities, and promotion consumer remain unwired. Production
promotion currently validates only staging deploy evidence, so no current run
may claim product qualification from this document alone.
