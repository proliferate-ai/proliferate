...
execution. A row may remain `planned`; a row marked `implemented` or `blocking`
must name its concrete collector, collected test id, lanes, gate, and evidence
status, and CI validates that mapping. Full qualification additionally requires
...
release policy, which derives required rows from the complete executable runner
registry and rejects every missing, non-green, or duplicate scenario/lane. The
scheduled local/source-built run uses signal policy, staging is dispatch-only,
and the self-host artifact job is only a post-publish diagnostic. The target
ledger remains explicitly `planned` until each row's concrete execution mapping
is audited, and trusted CI does not yet populate or consume every exact-artifact
identity in the summary. The complete Tier 3/Tier 4 mapping, signed product-wide
aggregator, target-manifest hash, and promotion consumer remain unwired.
Production promotion currently validates only staging deploy evidence, so no
current run may claim product qualification from this document alone.
