...
| Model gateway add-on: `--profile agent-gateway` + LiteLLM → real agent response | 3 | tests/release/src/scenarios/selfhost/t3-sh-3.ts (T3-SH-3; SSH to the standing box, brings the LiteLLM gateway up with the profile, makes a real cheapest-model completion; blocked until standing-box + gateway/provider keys are provisioned) |
