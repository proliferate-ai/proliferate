...
| Existing user warm path: reopen, pause (inaccessible), resume, state intact | 3 | tests/release/src/scenarios/t3-prov-2.ts (T3-PROV-2; local/single-org: expected-fail after verifying GET + wake. **Staging (first honest run 2026-07-09): the real end-to-end proof of the staging lane** — the durable user authenticates via the rotating product session (staging-session.ts) and GET /cloud-sandbox reads back over the live staging API; the mutating pause/wake half is deferred from the first staging pass to protect the SHARED durable sandbox → expected-fail) |
| Cloud workspace pause/resume/connect on real E2B | 3 | tests/release/src/scenarios/t3-prov-2.ts (T3-PROV-2; staging wake/pause deferred until a dedicated non-shared staging fixture exists — see above and the staging-lane runbook in .github/workflows/release-e2e.yml) |
