...
| Model gateway add-on: operator LiteLLM public inference path → scoped product enrollment → real agent response | 3 | — (the current T3-SH-3 direct-completion pointer bypasses product enrollment and is historical signal rather than qualification evidence) |
...
| Seat-based billing on Core orgs: invite/remove/re-invite reconciles Stripe quantity + proration grants, no double-grant | 2 | tests/intent/specs/billing/seats.spec.ts (T2-BILL-3) |
...
| Managed LLM credits: exhaustion/caps disable scoped LiteLLM keys; explicit LLM auto-top-up succeeds or declines independently from compute overage; user API key remains an alternate path | 2 | — (the current overage spec does not satisfy the complete T2-BILL-6/14/15 contract) |
...
| Credits consumed properly: real session → LiteLLM spend-log/import/debit for managed LLM plus compute segment/meter events; Stripe webhook delivery live on staging | 3 | tests/release/src/scenarios/t3-bill-1.ts (legacy pointer covers only a partial meter probe; LiteLLM import and the complete compute provider path remain qualification gaps) |
...
| Managed-credit exhaustion disables the scoped LiteLLM key; explicit LLM auto-top-up recovers only after successful payment, while user API key remains a separate route | 3 | — |
| Compute overage meters exact amounts to cap then hard-blocks; its switch never authorizes LLM top-up, charge, or grant | 3 | — |
