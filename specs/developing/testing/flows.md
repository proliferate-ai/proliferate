...
| Desktop artifact chain valid per release (server redirect follows, CDN manifest fresh, every shipped platform present and artifact HEAD 200, tag contains the SHA) | 4 | tests/release/src/scenarios/upgrade/t4-sh-2.ts (T4-SH-2 partial post-publish diagnostic; pure HTTP + git and wired for nightly/manual execution in `.github/workflows/release-e2e-selfhost.yml`. It resolves the actually shipped stable version and independent tag creation date, compares stable and immutable versioned manifests, enforces the shared updater-platform contract, and checks one-way exact-SHA ancestry when supplied. It is not a release gate; trusted pre-promotion candidate-manifest/signature/checksum validation and evidence consumption remain qualification gaps.) |
