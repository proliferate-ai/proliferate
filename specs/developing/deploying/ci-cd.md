...
- Contributor preparation and readiness follow the canonical
  [pull-request procedure](../process/pull-requests.md). This document owns the
  mechanical PR-metadata enforcement and downstream release behavior.
...
The canonical [pull-request procedure](../process/pull-requests.md) defines how
contributors select titles and labels and when a PR is ready. The
`.github/workflows/pr-metadata.yml` workflow enforces the machine-checkable
subset of that policy. Generated GitHub release notes consume those titles and
labels; this document owns the downstream release behavior and the repository
label provisioning required for enforcement.
