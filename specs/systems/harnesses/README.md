...
Not owned though co-located (see Fences): `auth/`, `route_auth/`, `status/` — the runtime half of **agent_auth**.
...
| Credential vault, selections, `state.json` delivery, route-auth rendering, gateway plans, the per-harness status documents — `domains/agents/{auth,route_auth,status}` | **agent_auth** (control-plane spec with a runtime section; today [AGENT_AUTH.md](../agent_auth/README.md)) |
...
└── (auth/, route_auth/, status/)           → agent_auth runtime section (fenced, see §7)
