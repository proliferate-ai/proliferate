...
Native integrations answers one question: *which specific pieces of the user's native harness environment has the user explicitly re-admitted into Proliferate sessions?* The answer is a per-workspace selection set, default empty. Selected pieces are never re-admitted by un-neutralizing the harness's own config; they are re-materialized by Proliferate as session MCP bindings and injected through ACP `session/new` like every other binding — enumerable, auditable, identical mechanics for every harness kind.
...
| --- | --- | --- |
| `native_integration_selections` — which discovered integrations are enabled, per workspace and agent kind | new migration under [persistence/sql/](../../../anyharness/crates/anyharness-lib/src/persistence/sql) | the selection API only |
...
CREATE TABLE native_integration_selections (
    ...
    UNIQUE (workspace_id, agent_kind, integration_id)
);
```
...
At session launch, when `native_integration_selections` has rows for (workspace, agent kind):

```text
resolve_native_launch_extras(workspace, record):
    selections = load_selections(workspace_id, agent_kind)     -- absence of rows: return empty
...
**The absence of rows is the absence of passthrough.** Zero selection rows for (workspace, agent kind) means the launch is exactly today's launch. No default-on bundle, ever — these capabilities include desktop control.
...
One section in the integrations pane — "From your Codex / Claude Code setup" — listing discovered integrations with per-row toggles writing `native_integration_selections`, bundles first with their own consent copy (Computer Use states plainly that the agent will control the local desktop and that macOS approval prompts come from the vendor's client). Unavailable rows render disabled with `unavailable_reason` (e.g. "Install the Codex desktop app and enable Computer Use once to provision it"). The pane is a thin editor over this system's contract, per [settings](../settings/README.md).
...
> kind (recommended — matches how product MCP attachment scopes today, and a
> desktop-control grant is naturally per-repo); (b) global per agent kind;
> (c) per-session with workspace default. Recommendation: (a), with (c) as a
> later additive.
...
- [ ] `domains/agents/native_integrations/` module, migration, contract routes, settings pane section — the build itself.
