...
native-only product standards (supervisor/worker convergence ownership,
native-specific build/release/test mechanics) → `specs/systems/desktop-host/desktop-native.md`.
It does not own cross-plane managed-runtime orchestration or deployment
→ `specs/systems/harnesses/managed-runtime.md`.

## Mental model

Desktop and Web mount the same compiled product implementation through a typed
host boundary. The Desktop host is a thin native shell that provides
Desktop-specific capabilities through `DesktopBridge` while the Web host passes
`desktop: null`. The boundary keeps raw Tauri, browser auth, vendor SDKs, and
native process management out of the shared product while preserving
Desktop-only product lifecycles behind the optional bridge.
...
the Desktop product into `@proliferate/product-client`.
...
None yet. The boundary is structural, not a source of runtime invariants that
can be violated.
...
None known. The extraction is complete and both hosts mount the same compiled
ProductClient.
