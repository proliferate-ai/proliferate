...
| The local API grows `GET /status`, `GET /status/stream`, `GET /methods`; the server grows `GET /seats/usage` + `POST /seats/{id}/limit-hit`; events grow `agent_seat_minted/limit_hit/rotated` | None of these exist | **Landed**: `GET /status`, `GET /status/stream`, `GET /methods` ship on the runtime local API (slice 3); `POST /seats/{id}/limit-hit` (slice 2), `GET /seats/usage` + the pane-open refresh (slice 4), and the `agent_seat_minted/limit_hit/rotated` events (slices 1–2) |
...
| Zero rows = unconfigured, with a migration for today's native users | Zero rows = native and the harness launches on its own login — retiring the convention converts working native setups into refusals | **Ruled 2026-08-27: the refusal cutover is CANCELLED — native is a permanent supported method.** A working native login keeps launching natively indefinitely; adding a managed method is an optional upgrade. The one-time settings prompt and the legacy-flag migration are not built. The status document already carries the state honestly (`native` methods row with `detected` + optional `offer: "mint_seat"`, `applied: null`, probe verdict), the client words native-detected + nothing-applied + probe-green as a healthy terminal ("Using your own login"), and the mint offer stays an optional upgrade affordance, never a nag |
