...
| Grok authenticates through its pane | **Landed (slice 6a).** The vendor CLI was always in the managed tree (`npm install --prefix agent_process @xai-official/grok` writes `node_modules/.bin/grok`, the same binary as the ACP sidecar, with working `login`/`--oauth`/`--device-auth`); login resolution's npm rung searched only the pre-lockfile `registry_npm/` layout, so every rung missed | Resolution gained the current managed-npm rung (`agent_process/node_modules/.bin/<program>`); the launcher (`grok-launcher`, baked `agent stdio` args) is deliberately NOT a login rung; the login declaration stays |
...
| Zero rows = unconfigured, with a migration for today's native users | Zero rows = native; the bridge mechanics are in (slice 6a) but the convention itself has not flipped: `route_auth::ABSENT_HARNESS_POLICY` is still `Native` | **The bridge landed (slice 6a):** the runtime's one-time seed pass grants a machine-local legacy flag (`agent-auth/native-bridge.json`) to every harness absent from the applied document and natively authenticated at that moment; `resolve_profile_bridged` is the one seam every launch resolves through; acting (a document naming the harness, or the pane prompt's dismiss via `DELETE /v1/agent-auth/native-bridge/{kind}`) clears the flag. Remaining: the refusal cutover flips `ABSENT_HARNESS_POLICY` to `Refuse` (slice 2's typed-refusal surface); the prompt's pixels are the design pass's call |
