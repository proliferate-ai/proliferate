...
- Clone/fetch/push mechanics and the disk they land on:
  [content.md](../environments/README.md). This document ends where the
...
- The product wire to the runtime:
  [gateway.md](../environments/README.md) — git traffic deliberately never
...
- VM dies: the lease dies with it; the replacement VM gets a fresh lease
  at its first materialization ([content.md](../environments/README.md)'s
