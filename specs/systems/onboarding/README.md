...
| Agent auth + gateway (Cloud `auth`, `agent-gateway`) | first-run adoption posts selections; the setting-up card is state-bound, never timed — it completes when every adopted agent reaches a terminal state in the runtime's status document | [use-first-run-auth-adoption.ts](../../../apps/packages/product-client/src/hooks/agents/lifecycle/use-first-run-auth-adoption.ts), [use-auth-setup-onboarding-evidence.ts](../../../apps/packages/product-client/src/hooks/agents/lifecycle/use-auth-setup-onboarding-evidence.ts) |
