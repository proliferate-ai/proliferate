...
| `AnchoredCommandPopover` | [AnchoredCommandPopover.tsx](../apps/packages/product-client/src/primitives/AnchoredCommandPopover.tsx) | Popover raised by a command rather than a control — full anchored-popover chrome/dismissal with no trigger element, anchored to a fixed point near the top chrome (command palette, deep link, empty-state entry points). |
