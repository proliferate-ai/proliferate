...
| `Textarea` | [Textarea.tsx](../apps/packages/product-client/src/primitives/Textarea.tsx) | Multi-line text input (default/ghost/flush variants). Deliberately has no mono/code variant: prose inputs render in the sans stack (PRO-153); genuine code inputs style mono at the call site (see the secrets/API-key editors). |
