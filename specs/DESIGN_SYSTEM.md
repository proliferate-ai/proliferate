...
| Component | Path | Purpose |
| --- | --- | --- |
| `ActionCard` | [ActionCard.tsx](../apps/packages/product-client/src/primitives/patterns/ActionCard.tsx) | Compact pressable card with a full-card primary `Button`, leading/title/description anatomy, and independent trailing and secondary-action slots. Promoted from Home's actionable onboarding tiles when the Home suggestion grid became the second real appearance; status-only onboarding and evidence cards keep their separate noninteractive anatomy. |
