...
- **`product-client/src/components/patterns/`** — domain-aware patterns. Same composition rule
  ...
  `PrStatusBadge`, `ProductPageShell`, `SettingsPageHeader`, and the `secrets/`
  sub-tree live here for that reason, not because they belong to a feature
  folder.
