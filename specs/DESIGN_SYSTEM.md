...
- **`product-client/src/components/patterns/`** — domain-aware patterns. Same composition rule
  ...
  [packages/README.md](frontend/packages.md)). `BillingGateState`, `ModelTable`,
  `PrStatusBadge`, `ProductPageShell`, and the `secrets/` sub-tree live here for
  that reason, not because they belong to a feature folder. `SettingsPageHeader`
  is the one row that is not here for that reason: it is a shape-only settings
  kit member parked here until the settings slice retires it.

  Admission to this tier is mechanical: the component's public props must reference a domain noun type — imported from `#product/domain/**` or `#product/lib/domain/**`, or a locally declared view type that the re-audit relocates there — AND the component must compose two or more library components (`SecretManagementPanel` passes, with consumers in the personal, organization, and repo secrets panes). Anything less is a presenter function feeding a pure pattern instead — the mapping still gets exactly one home, beside its types under `#product/lib/domain/**`, and the library stays smaller. Most current rows predate this test and are grandfathered pending re-audit — see Current Gaps. This tier is a shelf, not a landfill; the settings kit's descent into `primitives/patterns/settings/` already shrank it, and the remaining rows are the next re-audit's subject.
...
| Component | Path | Purpose |
| --- | --- | --- |
| `BillingGateState` | [BillingGateState.tsx](../apps/packages/product-client/src/components/patterns/BillingGateState.tsx) | Billing gate panel plus `BillingBalanceNotice` inline banner; `billingGateView` is the single mapping from typed start-block reasons to a view. |
...
- Kit directories cover four of the named kits, not all of them: the composer, toast, sidebar, and settings kits each own a subdirectory under `primitives/patterns/`, but two kits remain split across tiers against the kit-cohesion rule — the toast kit's positioner (`Sonner`) is a root primitive outside `patterns/toast/`, and one settings member (`SettingsPageHeader`) is still in `components/patterns/`, left there deliberately because the settings slice retires it in favour of `PageHeader` `variant="flat"` rather than moving it. The tabs and panel kits named in the closed kit set have no members yet. Nothing mechanical enforces that a kit member lands in its kit directory.
