...
| `Badge` | [Badge.tsx](../apps/packages/product-client/src/primitives/Badge.tsx) | Label/status chip on two independent axes: `tone` (the seven semantic tints) × `size` (`default` bordered pill / `micro` square count-chip for dense chrome — tighter radius and padding, no visible edge, flat `muted` fill for the neutral tone). Micro was promoted from the git review selectors' local `GitReviewCountChip` plus `LoopsPanel`'s native/emulated chip. |
...
- The rule of two is still review-enforced: no script detects a second implementation of a shape, so several duplicated shapes (roster rows, card shells, status dots, disclosure state machines, inline notices) whose shared vocabulary now exists but whose call sites are not migrated onto it yet are caught by review only. The at-least-one-call-site rule and review check 3 are now mechanical — [check_component_library.py](../scripts/check_component_library.py) fails on a dead index row and on a hand-rolled overlay role — but each ratchets from a written allowlist rather than from zero: `RangeSlider`, `AuthProviderButton` and `ListRow` are recorded there as dead vocabulary awaiting a founder retirement ruling, and the surviving overlay shells (the file-tree overlay, the delegated-agent hover card, the composer inline listbox, two diff expanders, plus the library's own recorded `role="button"` trades on `RosterRow` and `SidebarRowSurface`) each carry the reason they still stand.
