...
- **`product-client/src/components/patterns/`** — domain-aware patterns. Same composition rule
  ...
  `PrStatusBadge`, `SettingsPageHeader`, and the `secrets/` sub-tree live here
  for that reason, not because they belong to a feature folder.
...
| `PanelHeaderEntry` | [panel/PanelHeaderEntry.tsx](../apps/packages/product-client/src/primitives/patterns/panel/PanelHeaderEntry.tsx) | Panel kit — the canonical entry of a right-panel header strip: leading glyph, truncating label, optional trailing slot, dirty dot, hover-revealed close. Owns `role="tab"`/`aria-selected`/roving `tabIndex` and its own state stack instead of the `.right-panel-tab-system` descendant CSS. Promoted from four hand-rolled header buttons. |
