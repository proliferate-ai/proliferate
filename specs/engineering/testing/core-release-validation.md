...
| `T2-MODELREG-1` | Distribution/presentation catalog and executable target observation remain distinct. `HarnessLaunchOptions` covers non-empty, observed-empty, same-basis failure, basis invalidation, and unknown IDs; local and cloud pickers preserve exact membership; create reloads and exact-validates complete intent with no alias, visibility override, seed, or first-row fallback. |
...
| ID | Required validation |
| --- | --- |
| `T2-WF-1` | UI definition creation/edit, live reference validation, manual launch, and the invocation-freeze seam. Covered by the tier-2 seam spec `workflow-trigger-seam.spec.ts` (the gen-1 collector `workflow-runs.spec.ts` and its lane are deleted; see `specs/engineering/testing/scenarios.md#T2-WF-1`). |
...
| ID | Required validation |
| --- | --- |
| `T3-CHAT-1` | In the Desktop-local runtime world, every probed, registry-supported harness installs its trusted pin, reads the target's current `HarnessLaunchOptions`, selects an observed qualification model, persists exact intent, confirms it in the ready live snapshot, completes a bounded turn, and reopens history. Managed cloud and self-host use one representative harness to prove their distinct target-copy wiring rather than repeating the harness Cartesian product. A missing/empty/failed observation is incomplete or typed failure, never catalog fallback. Under the temporary 2026-07-22 ruling, Grok chat-spend is unsupported and cannot satisfy this row. |
...
| `T3-CFG-1` | Create-time `modelId` and generic `controlValues` derive only from the target's `HarnessLaunchOptions`, are stored atomically as `ResolvedLaunchIntent`, and are confirmed before ready. In an existing session, mutate every model/control/value advertised by `SessionLiveConfigSnapshot`, read back exactly, apply on the next turn, and survive reconnect; stale/unknown values fail without mutating state. Codex proves independent Mode and Access, including `agent-full-access` and absence of obsolete `full-access`. |
...
| `T3-SUBAGENT-1` | Workspace MCP exposes exactly 20 tools and proves one valid call for each against eligible Standard ordinary, delegated, and promoted/restarted sessions; `InternalOnly` and Cowork sessions receive no Workspace attachment. Request-time authority denies delegated creation and invalid parent, relationship, or cross-workspace targets; per-turn product context changes on promotion; create, pin, unpin, send, Close, Open, Promote, and durable completion relay preserve their documented correlation and replay behavior. Pin/unpin emits a runtime-owned session event before success; ACP result content cannot mutate client preferences. No legacy Subagents MCP name or delegated-agent wake schedule remains, while Cowork wake mobility still passes. |
...
| `T3-DELEGATE-UI-1` | In the Desktop product renderer, live subagent, Cowork, plan-review, and code-review work agrees across the Agents pane/popover, attached review/Cowork tabs, sidebar, transcript receipts, and details. Same-workspace subagents remain right-pane-only until Promote opens the same durable session as an ordinary top-level tab. Canonical identity/status, hierarchy, breadcrumb, attention filtering, close-tab versus end-work confirmation, page reload, and transcript retention hold. |
...
| `T3-INT-1` | Every supported harness in the Desktop-local runtime world calls one real API-key integration through Product MCP. Managed cloud repeats the positive call with one representative harness to prove cloud materialization without duplicating the Cartesian product. Audit records exact provider/tool/outcome; deterministic lower tiers own disable/revoke and zero-upstream-call failure matrices. Under the temporary 2026-07-22 ruling, Grok integration-audit is unsupported because the live call emits no post-baseline product audit row. |
...
| `T3-BILL-7` | Compute exhaustion creates the owned hold and pauses/blocks real E2B without changing LLM credit. Direct create/ensure/resume, stale session, forced provider resume, old token, second member, workflow, and automation cannot bypass the gate; provider-side resume is re-paused. |
...
| `T4-MODELREG-1` | Harness pin/auth basis, launch-option contract, or saved-intent changes | N-1 saved intents remain opaque exact IDs under N and are revalidated against the selected target's current observation. Still-valid tuples launch unchanged; removed values surface typed repair. No alias, visibility override, seed, neighboring row, or fallback substitutes a value. Running sessions remain governed only by their own live snapshot. |
...
- the agent-auth and model-gateway platform contracts are now written
  ([agent-auth.md](../../systems/agent_auth/deep-dive.md),
