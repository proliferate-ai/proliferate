...
| --- | --- | --- |
| **staging** | THE standing e2e world for everything that deploys: the staging ECS stack, real LLM via the staging gateway, Stripe test mode. Continuously current once green-main auto-deploy lands; the battery runs against it nightly + pre-promote; its verdict is prod's door (observe mode first). Seeding: `tests/release/scripts/staging_session_seed.py` (in-VPC one-off). | alive but frozen at 0.4.23 since 08-21; the e2e session credential rotted (no rotation write-back — being rebuilt); lane was masked by `continue-on-error` (being stripped) |
