...
| Add-Repo flow entry: local/cloud branches render + desktop-web explains that native folder picking requires Desktop | 2 | tests/intent/specs/workspace-entry.spec.ts |
...
| Flow | Tier | Test pointer |
| --- | --- | --- |
| Every installed, observed harness × one cheap target-reported model via the gateway: send a message, get a real message back — local lane AND sandbox lane | 3 | tests/release/src/scenarios/t3-chat-1.ts (T3-CHAT-1; local lane green for claude, per-harness expected-fail for opencode — issue #1024; sandbox lane blocked on current_product_user; not yet routed through a dedicated gateway test key, see env-manifest.ts) |
...
| Config/harness option updates applied by a live agent: cycle every exact model/control/value in the session's full `SessionLiveConfigSnapshot` | 3 | tests/release/src/scenarios/t3-cfg-1.ts (T3-CFG-1; local lane green — verified round-trip on mode + model controls) |
...
| Model gateway real product path: enroll route (`state.json` only) → observe target launch options → create with exact model intent and no workspace-env injection (#1106) → confirm the ready live snapshot → real streamed turn | 3 | tests/release/src/scenarios/t3-gw-1.ts (T3-GW-1; local runtime lane, supplements T3-SH-3's direct completion — asserts the shipped workspace→observation→session→turn path over the gateway with the credential living only in `agent-auth/state.json`. Needs a reachable local AnyHarness runtime + RELEASE_E2E_GATEWAY_TEST_KEY/_BASE_URL; BLOCKED otherwise. The deterministic half proves absent IDs are rejected and exact observed IDs accepted through the runtime seam.) |
...
| Adaptive sign-in surface is truthful on the real deploy artifact: password login always advertised, github methods/availability agree | 3 | tests/release/src/scenarios/selfhost/t3-sh-5.ts (T3-SH-5; read-only against the standing box, RELEASE_E2E_SELFHOST_URL; no login attempted — this box's real admin credentials are not available to the scenario; BLOCKED without that var) |
...
| Desktop artifact chain valid per release (server redirect follows, CDN manifest fresh, every platform artifact HEAD 200, tag contains the SHA) | 4 | tests/release/src/scenarios/upgrade/t4-sh-2.ts (T4-SH-2; the 2026-07-09 incident test — pure HTTP + git, no box; .github/workflows/release-e2e-selfhost.yml runs on schedule/manual dispatch and exposes `workflow_call`, but no current release coordinator calls it, so the every-release gate remains a target requirement; self-hosting.md §5) |
...
| Trigger a workflow via UI → exactly one invocation PUT carrying the frozen definition + placement, UI survives the runtime-plane PUT failing (stops at the runtime seam) | 2 | `tests/intent/specs/workflow-trigger-seam.spec.ts` (`T2-WF-1`; see `specs/engineering/testing/scenarios.md#T2-WF-1`) |
...
| Out of credits: sandbox paused and not accessible — **including every bypass route** (direct API ensure, stale session, webhook race, pre-exhaustion key, other org member, trigger-driven start) | 3 | tests/release/src/scenarios/t3-bill-2.ts (T3-BILL-2; exhaustion setup via drain-grants green; enforcement + 6-route bypass sweep blocked on github_link_required — cloud routes need a real sandbox + gate lift PR #1023) |
