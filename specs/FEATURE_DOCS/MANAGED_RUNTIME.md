...
**Binary versions are all the heartbeat carries.** The agent catalog ships inside the runtime binary, so there is no catalog version on the wire. A runtime binary update delivers new catalog pins by definition.
