...
- steering durably promotes one pending prompt, resolves parked interactions as
  ...
  next
- cancellation-delivery failure after a committed promotion returns a typed
  degraded outcome; repeating the steer for that stable `seq` skips promotion
  and retries only active-turn interruption
