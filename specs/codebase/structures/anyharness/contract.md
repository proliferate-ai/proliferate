...
or removals. A steer can therefore report the typed 503
`PENDING_PROMPT_STEER_DEGRADED` after its promotion has committed when delivery
of the active-turn cancellation fails. Repeating that steer for the same
stable `seq` does not promote or emit the reorder again; it only retries the
interruption.
