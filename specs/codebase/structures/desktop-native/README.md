...
| Desktop dispatch worker | `commands/cloud_worker.rs` owns the optional Proliferate Worker launcher process. It is separate from the always-on AnyHarness sidecar. The app exit event stops and reaps the tracked launcher explicitly before Tauri terminates the process; a fresh credential rotation may take over only an untracked Worker whose executable and exact private config path both match the target. |
