...
3. Load runtime access: `anyharness_base_url` plus the bearer and data-key
   ...
   cold-access law ([sandbox-lifecycle.md](sandbox-lifecycle.md)), not
   ...
   runs is [sandbox-access.md](sandbox-access.md)'s contract.
...
| Git fetch/push | sandbox → GitHub | **Direct** (HTTPS + credential-helper lease) | The helper reads a local token file, no network hop through Cloud ([sandbox-github-auth.md](sandbox-github-auth.md)) |
...
| Materialization scripts | server → sandbox | **E2B exec channel** (provider SDK, API key) | Control plane; server-only credentials ([sandbox-lifecycle.md](sandbox-lifecycle.md)) |
...
- Upstream unreachable (VM killed, host stale): typed 502
  ...
  reset ([sandbox-lifecycle.md](sandbox-lifecycle.md)).
