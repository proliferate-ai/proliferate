...
| Agent auth platform | Harness auth source selection (`gateway`/`api_key` selections, native = no selection), key vault, `state.json` materialization, and fail-closed launch behavior. | No current document; the Bifrost-era `agent-auth.md` was removed as stale and a rewrite is planned. Owning code: `server/proliferate/server/cloud/agent_gateway/`, `anyharness-lib/src/domains/agents/route_auth/`. |
