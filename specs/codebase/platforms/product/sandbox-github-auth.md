...
by) belongs to [sandbox-content.md](sandbox-content.md).
...
- Clone/fetch/push mechanics and the disk they land on:
  [sandbox-content.md](sandbox-content.md). This document ends where the
...
- The product wire to the runtime:
  [sandbox-gateway.md](sandbox-gateway.md) — git traffic deliberately never
...
[sandbox-content.md](sandbox-content.md)) first runs
...
- VM dies: the lease dies with it; the replacement VM gets a fresh lease
  at its first materialization ([sandbox-content.md](sandbox-content.md)'s
