...
under `/auth/**`. Organizations owns OrganizationMembership mutations.
...
- **Linked providers** are external OAuth identities and grants attached to the
  account. GitHub, Google, and Apple identities are linked providers.
...
There is one carve-out to the GitHub readiness gate in `current_product_user`:
