...
[FEATURE_DOCS/WORKFLOWS.md](../../../../FEATURE_DOCS/WORKFLOWS.md), which stays as
...
| --- | --- | --- |
| Session lifecycle, event log, pending-prompt queue | runtime `sessions` ([sessions.md](../../../../anyharness/sessions.md)) | a node *is* a session; this system observes turn ends through `SessionExtension` and never writes session rows |
| Worktree creation, repo-root registry, destruction | runtime `workspaces` ([workspaces.md](../../../../anyharness/workspaces.md)) | `workflow_placement.rs` lives in workspaces; this system supplies the run id and mode |
| Harness selection, credentials, launch options | harnesses / [agent_auth](../agent_auth/README.md) / [MODELS.md](../../../../FEATURE_DOCS/MODELS.md) | a node's `model` is a pass-through config; resolution is the launcher's |
...
| Cross-environment spawn, run results, budget envelopes | [runs.md](../runs/README.md) (target) | the CP run record; today the runtime run is the only run object |
...
| Client orchestration-hook folders named `hooks/<domain>/workflows/` | frontend structure ([hooks.md](../../../../frontend/hooks.md)) | a naming collision, not this system: only the product folders in the code map are Workflows UI |
...
- [ ] **Placement is runtime-local.** `repoConfigId` is a desktop repo-root
      id; task-class cloud placement ([runs.md](../runs/README.md)) needs a placement
      the CP can resolve.
- [ ] **No run record at the CP.** The runtime run is the only run object; the
      ...
      the seam [runs.md](../runs/README.md) draws — bucket-4 when it lands.
