...
supersedes [FEATURE_DOCS/AGENT_AUTH.md](../../../../FEATURE_DOCS/AGENT_AUTH.md) as the
...
| --- | --- | --- |
| harness auth vocabulary (slots, env names, login policy, `providerConfig` kinds) | agent distribution ([agent-distribution.md](../../../platforms/product/agent-distribution.md), [registry.json](../../../../../catalogs/agents/registry.json)) | declare-vs-apply: this system applies what the registry declares |
| the per-(subject, harness) gateway virtual key + budget status | model gateway ([MODELS.md](../../../../FEATURE_DOCS/MODELS.md) §Model gateway; code co-resident in `server/agent_auth/`) | rendered as an opaque `gateway` source; `budget.py`'s exhaustion predicate withholds key material at render |
...
| --- | --- | --- |
| Enrollment, LiteLLM teams/users, virtual keys, access groups, budgets, top-ups, usage import, verification, free credits | model gateway ([MODELS.md](../../../../FEATURE_DOCS/MODELS.md) §Model gateway) | this system renders the minted key as an opaque value; which models the key can see is proxy-side. The code is co-resident in `server/agent_auth/` (see code map) |
| Registry auth vocabulary, readiness *projection* | agent distribution ([agent-distribution.md](../../../platforms/product/agent-distribution.md)) | declare vs apply |
...
| Model/launch-option observation, session live config | [MODELS.md](../../../../FEATURE_DOCS/MODELS.md) | this system never filters models |
...
| User login, JWTs, org membership | product auth ([auth/README.md](../auth/README.md)) | a different auth |
...
- [ ] **Resolution order for headless subjects.** `run override → subject
      ...
      policy only *restricts*. Task-class runs ([runs.md](../runs/README.md)) need org
