...
> 202, read-back showed `secureFields.authorization_credentials=true`
> (server-side encrypted), and the route tree was byte-identical across the
> add/remove cycle. The outcome is unchanged: a dedicated Bearer contact
> point, present but unreferenced. The operator script performs the
> read-modify-write with a post-write verification (route tree byte-identical,
> other receivers untouched, credential stored encrypted) and restores the
> pre-write config on any verification failure.
...
6. create or update only the named tracker contact point;
