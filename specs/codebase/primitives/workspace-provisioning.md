...
- choosing the owner scope and repo/branch/worktree identity for a new managed
  workspace
- creating or reusing the sandbox profile, primary target, active slot, and
  durable `cloud_workspace` row
- creating exposure and projection rows that make the workspace visible and
  commandable
- queuing materialization/session commands with the right preflights and slot
  fences
...
- profile/target/slot schema details, owned by
...
2. [sandbox-provisioning.md](sandbox-provisioning.md) for profile, target, slot,
   and `cloud_workspace` creation invariants.
...
Cloud product ledger
  -> sandbox_profile, cloud_target, cloud_sandbox slot, cloud_workspace
...
runtime materialization ledger
  -> AnyHarness workspace id, worktree path, materialization generation
```
...
5. Server ensures or schedules active slot provisioning; E2B work remains
   background-owned by sandbox provisioning.
...
8. Server queues materialize/start/send commands with cloud_workspace_id,
   ...
   revision, and slot identity.
9. Wake-required commands return quickly; the wake job runs asynchronously.
10. Worker leases the command only under the active slot fence.
11. Worker materializes the workspace and echoes cloud_workspace_id plus the
    AnyHarness workspace id in the result.
12. Server verifies the result against the Cloud row and slot fence before
    updating materialization/session state.
...
| Sandbox profile, target, slot, runtime access, and `cloud_workspace` foundation | [sandbox-provisioning.md](sandbox-provisioning.md) |
...
- Command leasing, result ingest, and materialization updates are fenced by the
  active cloud sandbox slot id and `slot_generation`.
...
| Slot stuck creating, worker enrollment missing, runtime access absent | [sandbox-provisioning.md](sandbox-provisioning.md), then [cloud-commands.md](cloud-commands.md) for wake/delivery state |
...
| Worker result references unknown workspace or stale slot | [sandbox-provisioning.md](sandbox-provisioning.md), [cloud-commands.md](cloud-commands.md) |
...
- tests that materialization commands carry `cloud_workspace_id`,
  ...
  agent-auth revision
- result-ingest tests that reject unknown `cloud_workspace_id` and stale slot
  generations
