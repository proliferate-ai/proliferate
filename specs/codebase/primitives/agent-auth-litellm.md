...
agent_auth_route_selection         # server-side auth state
  ...
  slot (default 'primary'),        # composition axis — see below
  route (native|api_key|gateway), api_key_id (fk, null),
  revision, created_at, updated_at
  UNIQUE (user_id, harness_kind, surface, slot)
  CHECK (surface='cloud' → route != 'native')

  SLOT SEMANTICS (2026-07-01 amendment, lands as PR 13): single-source
  harnesses (claude/codex/grok/gemini) always use slot='primary' — store
  layer enforces exactly one row; radio semantics. OpenCode is ADDITIVE:
  slot ∈ {'gateway','openai','anthropic','xai','google'}, each row an
  independent toggle (gateway VK + any number of direct provider keys,
  simultaneously), merged by the renderer into ONE opencode config that
  ADDS to — never replaces — the user's own local providers (verify
  config-layer merge in PR 6 gates). State-file contract carries slot and
  allows multiple entries per harness; opencode UI is per-source toggles,
  not a radio group.
...
## 13. Open Risks — RESOLVED 2026-07-01 via live compat matrix

Live-verified against LiteLLM main-stable (full recipes preserved in the
PR 2 smoke harness; original matrix at /tmp/harness-compat/MATRIX.md):

1. ~~Codex→Anthropic tool errors~~ RESOLVED: LiteLLM `/v1/responses`
   bridges to anthropic correctly, tool calls included. Codex gateway is
   NOT OpenAI-only. `wire_api="responses"`, `env_key` in provider config
   (no `codex login` needed), `--skip-git-repo-check` for exec mode.
2. ~~grok-build~~: grok works via `GROK_MODELS_BASE_URL` + `/v1/models`
   discovery; grok-named aliases to any upstream work; keep a grok-build
   alias in gateway config (cheap insurance for title-gen).
3. ~~Claude fidelity~~ VERIFIED: streaming + tool calls work via
   `/v1/messages?beta=true`. NEW HARD REQUIREMENT: adapters must sanitize
   ambient provider env (`CLAUDE_CODE_USE_BEDROCK`,
   `AWS_BEARER_TOKEN_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`,
   `ANTHROPIC_API_KEY`) — ambient Bedrock env silently reroutes the CLI.
   Proxy config needs VERSIONED model ids; set
   `ANTHROPIC_SMALL_FAST_MODEL` for sidecar calls.
4. OpenCode: works via `@ai-sdk/openai-compatible` provider + explicit
   models map (required, confirmed). CLI lingers post-completion —
   headless runners need timeouts.
5. Gemini: genai facade works at ROOT `/v1beta/...:generateContent`
   (NOT `/gemini` prefix). CLI needs `selectedType=gemini-api-key`
   settings + `GEMINI_CLI_TRUST_WORKSPACE=true`. Managed pool MUST carry
   a real Google upstream — cross-provider aliasing breaks
   (temperature+top_p conflict in genai→anthropic translation).

Remaining:

6. ~~Workspace-flow merge~~ RESOLVED: merged to main 2026-07-01; PR 5
   unblocked.
7. BYOK direct-everywhere means revoked raw keys persist in sandboxes until
   the next materialization pass — documented behavior, revisit if it bites.
8. OpenAI-family gateway path unverified locally (dev .env has an invalid
   OPENAI_API_KEY) — native wire format, low risk; verify on staging.
